...
declare
  ...
  v_override boolean;
begin
  if v_user is null or p_group_id is null or coalesce(trim(p_permission),'') = '' then return false; end if;

  select gm.id, gm.role::text
    into v_member_id, v_role
  from public.group_members gm
  where gm.group_id=p_group_id and gm.user_id=v_user and gm.status='active'
  limit 1;
