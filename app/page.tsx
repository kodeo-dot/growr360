...
function buildSimplePdf(title:string,rawLines:string[],photos:PdfPhoto[]=[]){
  const pageW=595,pageH=842,margin=44,contentW=pageW-margin*2;
  ...
  const header=()=>{pageNo++;commands=["q","1 1 1 rg","0 0 595 842 re f","0.12 0.31 0.18 rg","0 746 595 96 re f","Q",pdfText(44,808,"GROWR360",13,true,"1 1 1"),pdfText(44,784,"GESTIÓN AGRÍCOLA",7.5,true,"0.78 0.89 0.80")];const cleanTitle=wrapPdfLine(title,55);cleanTitle.slice(0,2).forEach((line,index)=>commands.push(pdfText(44,756-index*17,line,15,index===0,"1 1 1")));commands.push(pdfText(462,808,"DOCUMENTO",7,true,"0.78 0.89 0.80"),pdfText(462,792,new Intl.DateTimeFormat("es-AR").format(new Date()),9,true,"1 1 1"));y=712;};
  const footer=()=>{commands.push("0.88 0.91 0.88 RG","44 38 m 551 38 l S",pdfText(44,22,"Generado con Growr360 · growr360.com",7.5,false,"0.42 0.48 0.43"),pdfText(528,22,String(pageNo),7.5,true,"0.42 0.48 0.43"));};
  ...
  for(let i=0;i<rawLines.length;i++){
    ...
    if(isSection){need(42);y-=8;commands.push("0.92 0.96 0.92 rg",`${margin} ${y-24} ${contentW} 31 re f`,pdfText(margin+12,y-13,raw,8.5,true,"0.18 0.38 0.22"));y-=39;continue}
    const colon=raw.indexOf(":");
    if(colon>0&&colon<42){const label=raw.slice(0,colon).trim();const value=raw.slice(colon+1).trim()||"—";const wrapped=wrapPdfLine(value,54);const h=Math.max(34,20+(wrapped.length-1)*12);need(h+5);commands.push("0.96 0.97 0.96 rg",`${margin} ${y-h+4} ${contentW} ${h} re f`,pdfText(margin+12,y-12,label.toUpperCase(),7.2,true,"0.42 0.49 0.43"));wrapped.forEach((line,j)=>commands.push(pdfText(margin+168,y-13-j*12,line,9.5,j===0,"0.10 0.14 0.11")));y-=h+6;continue}
    const wrapped=wrapPdfLine(raw,78);need(wrapped.length*13+8);wrapped.forEach((line,j)=>commands.push(pdfText(margin+2,y-j*13,line,9,false,"0.18 0.22 0.19")));y-=wrapped.length*13+5;
  }
  ...
  photos.forEach((photo,index)=>{const imageBody=bytesToLatin1(photo.data);const imageId=add(`<< /Type /XObject /Subtype /Image /Width ${photo.width} /Height ${photo.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${photo.data.length} >>\nstream\n${imageBody}\nendstream`);const maxW=507,maxH=650;const scale=Math.min(maxW/photo.width,maxH/photo.height);const drawW=photo.width*scale,drawH=photo.height*scale;const x=(595-drawW)/2,yImg=110+(650-drawH)/2;const caption=wrapPdfLine(photo.name||`Foto ${index+1}`,62).slice(0,2);const body=["q","1 1 1 rg","0 0 595 842 re f","0.12 0.31 0.18 rg","0 746 595 96 re f","Q",pdfText(44,808,"GROWR360",13,true,"1 1 1"),pdfText(44,784,"REGISTRO FOTOGRÁFICO",7.5,true,"0.78 0.89 0.80"),pdfText(44,756,`Foto ${index+1} de ${photos.length}`,15,true,"1 1 1"),pdfText(44,711,"FOTOGRAFÍA ADJUNTA",8.5,true,"0.18 0.38 0.22"),"q",`${drawW.toFixed(2)} 0 0 ${drawH.toFixed(2)} ${x.toFixed(2)} ${yImg.toFixed(2)} cm`,`/Im1 Do`,`Q`,...caption.map((line,j)=>pdfText(44,82-j*12,line,9,j===0,"0.14 0.18 0.15")),"0.88 0.91 0.88 RG","44 38 m 551 38 l S",pdfText(44,22,"Generado con Growr360 · growr360.com",7.5,false,"0.42 0.48 0.43")].join("\n");const contentId=add(`<< /Length ${pdfLatin1(body).length} >>\nstream\n${body}\nendstream`);const pageId=add(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${fontId} 0 R /F2 ${fontBoldId} 0 R >> /XObject << /Im1 ${imageId} 0 R >> >> /Contents ${contentId} 0 R >>`);pageIds.push(pageId)});
...
function WorkOrdersView({groupId,userId,orders,records,fields,plots,campaigns,members,contractors,supplies,plan,canCreate,canEditAny,canEditOwn,canDeleteAny,canDeleteOwn,onSaved}:{groupId:string;userId:string;orders:WorkOrder[];records:RecordRow[];fields:Field[];plots:Plot[];campaigns:Campaign[];members:Member[];contractors:Contractor[];supplies:Supply[];plan:string;canCreate:boolean;canEditAny:boolean;canEditOwn:boolean;canDeleteAny:boolean;canDeleteOwn:boolean;onSaved:()=>void}){
 ...
 function printWorkOrder(){const source=document.querySelector(".work-order-sheet") as HTMLElement|null;if(!source)return;document.querySelectorAll(".work-order-print-host").forEach(node=>node.remove());const host=document.createElement("div");host.className="work-order-print-host";const clone=source.cloneNode(true) as HTMLElement;clone.querySelectorAll(".work-order-actions,.icon-button,.form-error").forEach(node=>node.remove());clone.querySelectorAll("img").forEach(img=>{const src=img.getAttribute("src");if(src?.startsWith("/"))img.setAttribute("src",`${window.location.origin}${src}`)});host.appendChild(clone);document.body.appendChild(host);const cleanup=()=>{host.remove();window.removeEventListener("afterprint",cleanup)};window.addEventListener("afterprint",cleanup,{once:true});const images=Array.from(clone.querySelectorAll("img"));const ready=images.map(img=>img.complete?Promise.resolve():new Promise<void>(resolve=>{img.addEventListener("load",()=>resolve(),{once:true});img.addEventListener("error",()=>resolve(),{once:true})}));Promise.all(ready).then(()=>requestAnimationFrame(()=>requestAnimationFrame(()=>window.print())))}
