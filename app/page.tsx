...
type View = "mapa" | "campos" | "contratistas" | "registros" | "monitoreos" | "napas" | "campanas" | "reportes" | "equipo" | "solicitudes" | "invitaciones" | "mas" | "configuracion" | "grupo" | "planes";
...
type Profile = { id: string; first_name: string; last_name: string; username: string; email: string; avatar_path?: string | null };
...
function AuthenticatedApp({ session }: { session: Session }) {
  ...
  const loadGroupData = useCallback(async (targetGroup: string, quiet = false) => {
    ...
    const [fieldResult, plotResult, recordResult, memberResult, cropResult, assignmentResult, colorResult, settingsResult, campaignResult, clientResult, supplyResult,contractorResult] = await Promise.all([
      ...
      supabase.from("group_members").select("user_id,role,status,profiles!group_members_user_id_fkey(id,first_name,last_name,username,email,avatar_path),member_permission_overrides(permission,allowed)").eq("group_id", targetGroup).eq("status", "active").order("created_at"),
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    const [profileResult, membershipResult, plansResult, subscriptionResult, usageResult] = await Promise.all([
      supabase.from("profiles").select("id,first_name,last_name,username,email,avatar_path").eq("id", userId).single(),
...
function formatDetailValue(key:string,value:unknown){if(key==="monitoring_priority")return <ImportanceDetail value={value}/>;if(key.endsWith("_levels"))return <SeverityDetail value={value}/>;if(typeof value==="boolean")return value?"Sí":"No";if(key.includes("depth")&&String(value).trim()!=="")return `${number(value as string|number).toLocaleString("es-AR",{maximumFractionDigits:2})} cm`;if(key.endsWith("_at")){const date=new Date(String(value));if(!Number.isNaN(date.getTime()))return new Intl.DateTimeFormat("es-AR",{dateStyle:"medium",timeStyle:"short"}).format(date);}return translatedDetailText(value);}
