...
function RealReportsView({ fields, plots, records, crops }: { fields: Field[]; plots: Plot[]; records: RecordRow[]; crops: Crop[] }) {
  ...
  const filtered = records.filter(row => (!fieldId || row.field_id === fieldId) && (!plotId || row.plot_id === plotId) && (!campaignId || row.campaign_id === campaignId) && (!type || effectiveRecordType(row) === type) && (!contractor || normalizeText(row.contractor ?? "") === normalizeText(contractor)) && (!crop || recordCrop(row).toLowerCase() === crop.toLowerCase()));
...
function ChartDrilldown({label,rows,metric,onClose}:{label:string;rows:RecordRow[];metric:string;onClose:()=>void}){const suffix=metricSuffix(metric);const [selected,setSelected]=useState<RecordRow|null>(null);return <section className="chart-drilldown"><header><div><span className="eyebrow">ORIGEN DEL DATO</span><h4>{label}</h4><p>{rows.length} registro{rows.length===1?"":"s"} · Tocá uno para ver toda la información</p></div><button className="icon-button" onClick={onClose}><X/></button></header><div>{rows.sort((a,b)=>b.record_date.localeCompare(a.record_date)).map(row=><button className="drilldown-record" key={row.id} onClick={()=>setSelected(row)}><div><strong>{recordType(effectiveRecordType(row))}</strong><small>{relation(row.fields)?.name||"Campo"} · {relation(row.plots)?.name||"Sin lote"}</small></div><time>{formatDate(row.record_date)}</time><b>{chartValue(row,metric).toLocaleString("es-AR",{maximumFractionDigits:2})}{suffix}</b><ChevronRight/></button>)}</div>{selected&&<RecordDetail record={selected} onClose={()=>setSelected(null)}/>}</section>}
