...
function AuthenticatedApp({ session }: { session: Session }) {
  ...
  const loadGroupData = useCallback(async (targetGroup: string, quiet = false) => {
    ...
    const productResult=workOrderResult.error?{data:[],error:null as any}:await supabase.from("work_order_products").select("id,work_order_id,input_id,product_name,dose,dose_unit,planned_quantity,actual_quantity,notes").eq("group_id",targetGroup);
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {!groupId ? <EmptyWorkspace onGroups={() => setGroupBrowserOpen(true)}/> : <>
        ...
        {view === "equipo" && <RealTeamView section="members" groupId={groupId} members={members} fields={fields} plots={plots} currentRole={activeMembership?.role ?? "producer"} canManage={hasPermission("manage_members") || activeMembership?.role === "admin"} onSection={setView} onSaved={() => void loadGroupData(groupId, true)}/>}
...
function InlineRecordDetail({record}:{record:RecordRow}){
  ...
  return <div className="inline-record-detail">
    ...
    <div className="inline-detail-groups">
      {inputs.length>0&&<div className="inline-inputs"><header><Leaf/><span><strong>Insumos aplicados</strong><small>{inputs.length} producto{inputs.length===1?"":"s"}</small></span></header><div>{inputs.map(input=><article key={input.index}><strong>{input.name||`Insumo ${input.index+1}`}</strong><span>{input.dose.toLocaleString("es-AR",{maximumFractionDigits:3})} {input.unit||"u"}/ha</span><small>{input.quantity.toLocaleString("es-AR",{maximumFractionDigits:2})} {input.unit||"u"} · costo {input.subtotal.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</small></article>)}</div></div>}
...
function WorkOrdersView({groupId,userId,orders,fields,plots,campaigns,members,contractors,supplies,plan,canCreate,canEditAny,canEditOwn,onSaved}:{groupId:string;userId:string;orders:WorkOrder[];fields:Field[];plots:Plot[];campaigns:Campaign[];members:Member[];contractors:Contractor[];supplies:Supply[];plan:string;canCreate:boolean;canEditAny:boolean;canEditOwn:boolean;onSaved:()=>void}){
 ...
 const blank=()=>({title:"",order_type:"sowing" as WorkOrderType,status:"pending",priority:"normal",scheduled_date:"",scheduled_end_date:"",campaign_id:campaigns.find(c=>c.status==="active")?.id??campaigns[0]?.id??"",field_id:"",plot_id:"",planned_area:"",assigned_to:"",contractor_id:"",instructions:"",notes:"",allow_member_edits:false,products:[] as {input_id:string;product_name:string;dose:string;dose_unit:string;planned_quantity:string}[]});
 const [draft,setDraft]=useState(blank);const [actualDate,setActualDate]=useState(new Date().toISOString().slice(0,10));const [actualArea,setActualArea]=useState("");
 const filtered=[...orders].sort((a,b)=>String(b.created_at??"").localeCompare(String(a.created_at??""))).filter(item=>{const active=["pending","in_progress"].includes(item.status);if(status==="active"&&!active)return false;if(status!=="all"&&status!=="active"&&item.status!==status)return false;return `${item.title} ${item.fields?.name??""} ${item.plots?.name??""}`.toLowerCase().includes(query.toLowerCase())});
 ...
 const stats={pending:orders.filter(o=>o.status==="pending").length,inProgress:orders.filter(o=>o.status==="in_progress").length,overdue:orders.filter(o=>["pending","in_progress"].includes(o.status)&&Boolean(o.scheduled_date)&&String(o.scheduled_date)<today).length,done:orders.filter(o=>o.status==="completed").length};
 ...
 function openEdit(item:WorkOrder){setSelected(item);setDraft({title:item.title,order_type:item.order_type,status:item.status,priority:item.priority,scheduled_date:item.scheduled_date??"",scheduled_end_date:item.scheduled_end_date??"",campaign_id:item.campaign_id,field_id:item.field_id??"",plot_id:item.plot_id??"",planned_area:item.planned_area==null?"":String(item.planned_area),assigned_to:item.assigned_to??"",contractor_id:item.contractor_id??"",instructions:item.instructions??"",notes:item.notes??"",allow_member_edits:Boolean(item.allow_member_edits),products:(item.work_order_products??[]).map(p=>({input_id:p.input_id??"",product_name:p.product_name,dose:p.dose==null?"":String(p.dose),dose_unit:p.dose_unit??"",planned_quantity:p.planned_quantity==null?"":String(p.planned_quantity)}))});setFormStep(0);setMode("form")}
 function selectPlot(plotId:string){const plot=plots.find(p=>p.id===plotId);setDraft(current=>({...current,plot_id:plotId,field_id:plot?.field_id??current.field_id,planned_area:plotId?String(plot?.arable_area??""):current.planned_area}))}
 const plannedTotal=(dose:unknown,area:unknown)=>{const d=number(dose as string|number);const a=number(area as string|number);return d>0&&a>0?d*a:null};
 async function save(event:FormEvent){
  event.preventDefault();setMessage("");
  if(formStep<4){setFormStep(step=>Math.min(4,step+1));return}
  if(!draft.title.trim()){setMessage("Escribí un título para la orden.");setFormStep(0);return}
  if(!draft.campaign_id){setMessage("Seleccioná una campaña.");setFormStep(1);return}
  if(draft.plot_id&&!draft.field_id){setMessage("Seleccioná el campo al que pertenece el lote.");setFormStep(1);return}
  if(draft.scheduled_date&&draft.scheduled_end_date&&draft.scheduled_end_date<draft.scheduled_date){setMessage("La fecha hasta no puede ser anterior a la fecha desde.");setFormStep(2);return}
  setBusy(true);
  try{
   const payload={group_id:groupId,campaign_id:draft.campaign_id,field_id:draft.field_id||null,plot_id:draft.plot_id||null,order_type:draft.order_type,status:draft.status,priority:draft.priority,title:draft.title.trim(),instructions:draft.instructions.trim()||null,notes:draft.notes.trim()||null,scheduled_date:draft.scheduled_date||null,scheduled_end_date:draft.scheduled_end_date||null,planned_area:draft.planned_area?Number(draft.planned_area):null,assigned_to:draft.assigned_to||null,contractor_id:draft.contractor_id||null,allow_member_edits:Boolean(draft.allow_member_edits),created_by:selected?.created_by??userId,updated_at:new Date().toISOString()};
   const result=selected?await supabase.from("work_orders").update(payload).eq("id",selected.id).select("id").single():await supabase.from("work_orders").insert(payload).select("id").single();
   if(result.error)throw result.error;const id=result.data?.id;if(!id)throw new Error("La orden no devolvió un identificador. Volvé a intentar.");
   if(selected){const removed=await supabase.from("work_order_products").delete().eq("work_order_id",id);if(removed.error)throw removed.error}
   const products=draft.products.filter(p=>p.product_name.trim());
   if(products.length){const productResult=await supabase.from("work_order_products").insert(products.map(p=>({group_id:groupId,work_order_id:id,input_id:p.input_id||null,product_name:p.product_name.trim(),dose:p.dose?Number(p.dose):null,dose_unit:p.dose_unit||null,planned_quantity:plannedTotal(p.dose,draft.planned_area)})));if(productResult.error)throw productResult.error}
   setMode(null);setSelected(null);await Promise.resolve(onSaved());
  }catch(error){setMessage(error instanceof Error?error.message:"No se pudo guardar la orden. Revisá los datos e intentá nuevamente.")}finally{setBusy(false)}
 }
 ...
 async function completeWithoutWorkOrderRpc(item:WorkOrder){const storedType=item.order_type==="soil_analysis"?"other":item.order_type;const details={...(item.planned_data??{}),source_work_order_id:item.id,work_order_title:item.title,record_kind:item.order_type==="soil_analysis"?"soil_analysis":item.order_type};const saved=await supabase.rpc("save_activity_record",{p_id:null,p_group_id:groupId,p_campaign_id:item.campaign_id,p_field_id:item.field_id??null,p_plot_id:item.plot_id??null,p_type:storedType,p_date:actualDate,p_worked_area:actualArea?Number(actualArea):item.planned_area??null,p_responsible_id:item.assigned_to??userId,p_contractor:item.contractors?.name??"",p_machinery:"",p_observations:item.notes||item.instructions||"",p_allow_member_edits:true,p_data:details});if(saved.error)throw saved.error;const recordId=typeof saved.data==="string"?saved.data:(saved.data as {id?:string}|null)?.id;if(!recordId)throw new Error("No se pudo identificar el registro generado.");await supabase.from("records").update({source_work_order_id:item.id}).eq("id",recordId);const updated=await supabase.from("work_orders").update({status:"completed",resulting_record_id:recordId,completed_by:userId,completed_at:new Date().toISOString(),actual_data:{actual_date:actualDate,actual_area:actualArea?Number(actualArea):item.planned_area??null},updated_at:new Date().toISOString()}).eq("id",item.id).select("id").single();if(updated.error)throw updated.error}
 async function complete(event:FormEvent){event.preventDefault();if(!selected)return;setBusy(true);setMessage("");try{const result=await supabase.rpc("complete_work_order",{p_work_order_id:selected.id,p_actual_date:actualDate,p_actual_area:actualArea?Number(actualArea):selected.planned_area??null,p_actual_data:{},p_products:(selected.work_order_products??[]).map(p=>({id:p.id,actual_quantity:p.planned_quantity}))});if(result.error){const missing=result.error.code==="PGRST202"||/schema cache|could not find the function/i.test(result.error.message);if(!missing)throw result.error;await completeWithoutWorkOrderRpc(selected)}setMode(null);setSelected(null);onSaved();}catch(error){setMessage(spanishError(error))}finally{setBusy(false)}}
 ...
 const selectedPlot=plots.find(p=>p.id===draft.plot_id);const selectedField=fields.find(f=>f.id===(draft.field_id||selectedPlot?.field_id));
 const formTitle=selected?"Editar orden":"Nueva orden";
 return <div className="page-content work-orders-page"><PageHead title="Órdenes de trabajo" text="Planificá, asigná y convertí la ejecución en un registro real."/>
  ...
  <section className="content-card work-order-directory"><header><div className="work-order-search"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar por orden, campo o lote"/></div><select value={status} onChange={e=>setStatus(e.target.value)}><option value="active">Activas</option><option value="all">Todas</option><option value="pending">Pendientes</option><option value="in_progress">En curso</option><option value="completed">Completadas</option><option value="cancelled">Canceladas</option></select>{canCreate&&<button className="primary-action" onClick={openCreate}><Plus/>Nueva orden</button>}</header><div className="work-order-list">{filtered.map(item=>{const meta=[item.scheduled_date?formatDate(item.scheduled_date):"",item.fields?.name??"",item.plots?.name??"",item.planned_area?`${number(item.planned_area).toLocaleString("es-AR")} ha`:""].filter(Boolean);return <button key={item.id} onClick={()=>{setSelected(item);setMode("view");setMessage("")}}><i className={`priority-${item.priority}`}/><div><span>{typeLabel(item.order_type)}{item.scheduled_date?` · ${formatDate(item.scheduled_date)}`:""}</span><strong>{item.title}</strong>{meta.slice(1).length>0&&<small>{meta.slice(1).join(" · ")}</small>}</div><em className={`status-${item.status}`}>{statusLabel(item.status)}</em><ChevronRight/></button>})}{!filtered.length&&<EmptyLine text="No hay órdenes para este filtro."/>}</div></section>
  {mode&&<div className="record-detail-backdrop"><article className={`record-detail-sheet work-order-sheet${selected?` status-${selected.status}`:""}`}><div className="print-only-brand-header"><img src="/favicon.svg" alt=""/><span>Growr<b>360</b></span></div><header><div><span className="eyebrow">ORDEN DE TRABAJO</span><h2>{mode==="form"?formTitle:selected?.title}</h2></div><button className="icon-button" onClick={()=>{setMode(null);setSelected(null)}}><X/></button></header>
   {mode==="form"?<form className="work-order-form work-order-wizard" onSubmit={save}>{message&&<p className="form-error work-order-error">{message}</p>}<div className="record-wizard"><WizardHead step={formStep+1} total={5} title={formTitle}/>
    ...
    {formStep===1&&<section className="wizard-card"><span>PASO 2</span><h3>Ubicación y superficie</h3><label>Campaña<select required value={draft.campaign_id} onChange={e=>setDraft({...draft,campaign_id:e.target.value})}><option value="">Seleccionar campaña…</option>{campaigns.map(c=><option key={c.id} value={c.id}>{c.name}{c.status==="active"?" · Activa":""}</option>)}</select></label><div className="form-pair"><label>Campo<select value={draft.field_id} onChange={e=>setDraft({...draft,field_id:e.target.value,plot_id:"",planned_area:""})}><option value="">Sin definir</option>{fields.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select></label><label>Lote<select value={draft.plot_id} onChange={e=>selectPlot(e.target.value)}><option value="">Sin definir</option>{plots.filter(p=>!draft.field_id||p.field_id===draft.field_id).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label></div><label>Superficie planificada (ha)<input type="number" min="0" step="0.01" value={draft.planned_area} onChange={e=>setDraft({...draft,planned_area:e.target.value})} placeholder={selectedPlot?`Sembrable del lote: ${number(selectedPlot.arable_area).toLocaleString("es-AR")} ha`:"Opcional"}/>{selectedPlot&&<small>Se completó automáticamente con la superficie sembrable del lote. Podés editarla.</small>}</label><WizardNavigation back={()=>setFormStep(0)} next={()=>setFormStep(2)}/></section>}
    ...
    {formStep===3&&<section className="wizard-card work-order-products-step"><span>PASO 4</span><h3>Insumos planificados</h3><p className="wizard-note">Opcional. El total se calcula automáticamente como dosis × superficie planificada.</p><section className="work-order-products"><header><div><strong>Productos</strong><small>{draft.planned_area?`Superficie base: ${number(draft.planned_area).toLocaleString("es-AR")} ha`:"Definí una superficie para calcular el total."}</small></div><button type="button" onClick={()=>setDraft({...draft,products:[...draft.products,{input_id:"",product_name:"",dose:"",dose_unit:"",planned_quantity:""}]})}><Plus/>Agregar</button></header>{draft.products.map((p,index)=>{const total=plannedTotal(p.dose,draft.planned_area);return <article className="work-order-product-editor" key={index}><header><strong>Producto {index+1}</strong><button type="button" aria-label="Quitar producto" onClick={()=>setDraft({...draft,products:draft.products.filter((_,i)=>i!==index)})}><X/></button></header><div className="work-order-product-fields"><label>Seleccionar insumo<select value={p.input_id} onChange={e=>{const supply=supplies.find(s=>s.id===e.target.value);const next=[...draft.products];next[index]={...p,input_id:e.target.value,product_name:supply?.name??p.product_name,dose_unit:supply?.unit??p.dose_unit};setDraft({...draft,products:next})}}><option value="">Producto manual</option>{supplies.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label>Producto<input placeholder="Nombre" value={p.product_name} onChange={e=>{const next=[...draft.products];next[index]={...p,product_name:e.target.value};setDraft({...draft,products:next})}}/></label><div className="form-pair"><label>Dosis<input type="number" min="0" step="0.001" placeholder="0" value={p.dose} onChange={e=>{const next=[...draft.products];next[index]={...p,dose:e.target.value};setDraft({...draft,products:next})}}/></label><label>Unidad<input placeholder="L, kg, cc…" value={p.dose_unit} onChange={e=>{const next=[...draft.products];next[index]={...p,dose_unit:e.target.value};setDraft({...draft,products:next})}}/></label></div><div className="work-order-product-total"><small>TOTAL PLANIFICADO</small><strong>{total!==null?`${total.toLocaleString("es-AR",{maximumFractionDigits:3})} ${p.dose_unit||"u"}`:"—"}</strong></div></div></article>})}{!draft.products.length&&<button type="button" className="work-order-add-first-product" onClick={()=>setDraft({...draft,products:[{input_id:"",product_name:"",dose:"",dose_unit:"",planned_quantity:""}]})}><Plus/>Agregar primer producto</button>}</section><WizardNavigation back={()=>setFormStep(2)} next={()=>setFormStep(4)}/></section>}
    {formStep===4&&<section className="wizard-card"><span>PASO 5</span><h3>Revisá la orden</h3><div className="record-review"><strong>{draft.title||"Orden sin título"}</strong><small>{typeLabel(draft.order_type)}{selectedField?` · ${selectedField.name}`:""}{selectedPlot?` · ${selectedPlot.name}`:""}</small>{draft.planned_area&&<small>{number(draft.planned_area).toLocaleString("es-AR")} ha planificadas</small>}{draft.scheduled_date&&<small>{formatDate(draft.scheduled_date)}{draft.scheduled_end_date?` → ${formatDate(draft.scheduled_end_date)}`:""}</small>}{draft.products.filter(p=>p.product_name.trim()).length>0&&<small>{draft.products.filter(p=>p.product_name.trim()).length} producto{draft.products.filter(p=>p.product_name.trim()).length===1?"":"s"}</small>}</div><label className="work-order-member-edits wizard-check"><input type="checkbox" checked={Boolean(draft.allow_member_edits)} onChange={e=>setDraft({...draft,allow_member_edits:e.target.checked})}/><span><strong>Permitir edición por el equipo</strong><small>Otros miembros con permiso de edición podrán modificar, iniciar o cancelar esta orden.</small></span></label><button type="button" className="wizard-back-only" onClick={()=>setFormStep(3)}>Volver a editar</button></section>}
   ...
   :mode==="complete"?<form className="work-order-complete" onSubmit={complete}><div className="planned-actual"><article><small>PLANIFICADO</small>{selected!.scheduled_date&&<strong>{formatDate(selected!.scheduled_date)}</strong>}{selected!.planned_area&&<span>{number(selected!.planned_area).toLocaleString("es-AR")} ha</span>}{!selected!.scheduled_date&&!selected!.planned_area&&<span>Sin fecha ni superficie planificada</span>}</article><ArrowRight/><article><small>REAL</small><label>Fecha<input required type="date" value={actualDate} onChange={e=>setActualDate(e.target.value)}/></label><label>Superficie (ha)<input type="number" min="0" step="0.01" value={actualArea} onChange={e=>setActualArea(e.target.value)}/></label></article></div>{plan!=="free"&&<div className="comparison-note"><TrendingUp/><span><strong>Comparación planificado vs. real</strong><small>{plan==="business"?"El plan Business conserva análisis avanzado y desvíos por recurso.":"El plan Pro conserva la comparación básica de fecha y superficie."}</small></span></div>}{message&&<p className="form-error">{message}</p>}<footer><button type="button" className="secondary-action" onClick={()=>setMode("view")}>Volver</button><button className="primary-action" disabled={busy}>{busy?<LoaderCircle className="spin"/>:<Check/>}Confirmar ejecución</button></footer></form>
   :selected&&<div className="work-order-detail"><div className="work-order-hero"><div><span className={`status-${selected.status}`}>{statusLabel(selected.status)}</span><h3>{typeLabel(selected.order_type)}</h3>{selected.instructions&&<p>{selected.instructions}</p>}</div><span className={`priority priority-${selected.priority}`}>{selected.priority==="urgent"?"Urgente":selected.priority==="high"?"Alta":selected.priority==="low"?"Baja":"Normal"}</span></div>{Boolean(selected.scheduled_date||selected.field_id||selected.plot_id||selected.planned_area||selected.assigned_to||selected.contractor_id)&&<div className="work-order-facts">{selected.scheduled_date&&<article><CalendarDays/><small>Fecha prevista</small><strong>{formatDate(selected.scheduled_date)}{selected.scheduled_end_date?` → ${formatDate(selected.scheduled_end_date)}`:""}</strong></article>}{(selected.field_id||selected.plot_id)&&<article><Sprout/><small>Ubicación</small><strong>{[selected.fields?.name,selected.plots?.name].filter(Boolean).join(" · ")}</strong></article>}{selected.planned_area&&<article><Grid2X2/><small>Superficie planificada</small><strong>{number(selected.planned_area).toLocaleString("es-AR",{maximumFractionDigits:2})} ha</strong></article>}{selected.assigned_to&&<article><Users/><small>Responsable</small><strong>{[selected.profiles?.first_name,selected.profiles?.last_name].filter(Boolean).join(" ")||selected.profiles?.username}</strong></article>}{selected.contractor_id&&<article><Tractor/><small>Contratista</small><strong>{selected.contractors?.name}</strong></article>}</div>}{Boolean(selected.work_order_products?.length)&&<section className="work-order-product-section"><h3>Insumos planificados</h3><div className="work-order-product-table"><div className="work-order-product-table-head"><span>Producto</span><span>Dosis</span><span>Total</span></div>{selected.work_order_products!.map(p=>{const total=plannedTotal(p.dose,selected.planned_area);return <div className="work-order-product-row" key={p.id}><strong>{p.product_name}</strong><span>{p.dose?`${number(p.dose).toLocaleString("es-AR",{maximumFractionDigits:3})} ${p.dose_unit??""}/ha`:"—"}</span><span>{total!==null?`${total.toLocaleString("es-AR",{maximumFractionDigits:3})} ${p.dose_unit??""}`:"—"}</span></div>})}</div></section>}{selected.notes&&<section className="work-order-notes"><h3>Notas</h3><p>{selected.notes}</p></section>}<div className="print-only-brand-footer">Orden generada con Growr360 · growr360.com</div><footer className="work-order-actions"><button onClick={()=>window.print()}><Download/>PDF</button><button onClick={()=>void share(selected)}><Link2/>Compartir</button>{mayEdit(selected)&&selected.status!=="completed"&&selected.status!=="cancelled"&&<button onClick={()=>openEdit(selected)}><Settings2/>Editar</button>}{mayEdit(selected)&&selected.status==="pending"&&<button onClick={()=>void changeStatus(selected,"in_progress")}><Activity/>Iniciar</button>}{canCreate&&selected.status!=="completed"&&selected.status!=="cancelled"&&<button className="primary-action" onClick={()=>{setActualDate(new Date().toISOString().slice(0,10));setActualArea(selected.planned_area?String(selected.planned_area):"");setMode("complete")}}><Check/>Completar</button>}{mayEdit(selected)&&!["completed","cancelled"].includes(selected.status)&&<button className="danger-text" onClick={()=>void changeStatus(selected,"cancelled")}>Cancelar orden</button>}</footer>{message&&<p className="form-error">{message}</p>}</div>}
  </article></div>}
 </div>
}
...
function ManagementView({ groupId, userId, fields, plots, campaigns, clients, contractors, crops, supplies, initialForm, initialRecord, onInitialRecordConsumed, onClose, onSaved }: { groupId: string; userId: string; fields: Field[]; plots: Plot[]; campaigns: Campaign[]; clients: ClientRow[]; contractors:Contractor[]; crops: Crop[]; supplies: Supply[]; canFields: boolean; canLots: boolean; canCampaigns: boolean; canRecords: boolean; initialForm:"field"|"campaign"|"client"|"contractor"|"record"; initialRecord: {plotId:string;type:string}|null; onInitialRecordConsumed:()=>void; onClose:()=>void; onMap: () => void; onSaved: () => void }) {
...
function RealTeamView({ section, groupId, members, fields, plots, currentRole, canManage, onSection, onSaved }: { section:"members"|"requests"|"invitations"; groupId:string; members: Member[]; fields:Field[];plots:Plot[];currentRole:string; canManage:boolean; onSection:(view:View)=>void; onSaved:()=>void }) {
  ...
  if(section==="invitations")return <div className="page-content team-subpage"><button className="page-back-button" onClick={()=>onSection("equipo")}><ChevronLeft/>Volver al equipo</button><PageHead title="Invitaciones" text="Creá accesos directos con un rol definido y administrá los enlaces pendientes."/>{message&&<p className={/(creada|copiado|revocada)/i.test(message)?"save-success":"form-error"}>{message}</p>}{invitationPanel}{!canManage&&<EmptyLine text="No tenés permiso para administrar invitaciones."/>}</div>;
  if(section==="requests")return <div className="page-content team-subpage"><button className="page-back-button" onClick={()=>onSection("equipo")}><ChevronLeft/>Volver al equipo</button><PageHead title="Solicitudes de ingreso" text="Revisá la identidad y el rol solicitado antes de sumar una persona."/>{message&&<p className={/(forma parte|rechazada)/i.test(message)?"save-success":"form-error"}>{message}</p>}{canManage?<section className="team-requests standalone"><header><div><span className="eyebrow">PENDIENTES</span><h3>Personas esperando aprobación</h3><p>Las solicitudes están separadas del listado de integrantes para trabajar con más claridad.</p></div><span className="request-count">{loadingRequests?<LoaderCircle className="spin"/>:requests.length}</span></header>{requests.length?<div>{requests.map(request=><article key={request.id}><div className="member-avatar">{initials(request.name||request.username||"Usuario")}</div><div className="request-person"><strong>{request.name||request.username||"Usuario"}</strong><span>{request.username?`@${request.username} · `:""}{request.email}</span><small>Solicita ingresar como {roleName(request.requested_role||"producer")} · {new Intl.DateTimeFormat("es-AR",{day:"numeric",month:"short",year:"numeric"}).format(new Date(request.created_at))}</small>{request.phone&&<small>{request.phone}</small>}</div><div className="request-actions"><button className="request-reject" disabled={busy===request.id} onClick={()=>void resolveRequest(request,false)}><X/>Rechazar</button><button className="request-accept" disabled={busy===request.id} onClick={()=>void resolveRequest(request,true)}>{busy===request.id?<LoaderCircle className="spin"/>:<Check/>}Aceptar</button></div></article>)}</div>:!loadingRequests&&<div className="requests-empty"><Check/><span><strong>Todo al día</strong><small>No hay solicitudes pendientes.</small></span></div>}</section>:<EmptyLine text="No tenés permiso para administrar solicitudes."/>}</div>;
