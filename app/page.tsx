...
function PublicLanding({ onLogin, onRegister }: { onLogin: () => void; onRegister: () => void }) {
  return <div className="public-site">
    ...
    <main>
      ...
      <section id="nosotros" className="about-section about-section-v2"><div className="about-copy"><span>QUIÉNES SOMOS</span><h2>El campo y la tecnología, hablando el mismo idioma.</h2><p>Growr360 nace para resolver problemas reales del trabajo agrícola argentino: conectividad limitada, información dispersa y herramientas complejas.</p><div className="about-purpose"><Leaf/><div><strong>Una mirada compartida</strong><small>Conocimiento agronómico para definir lo importante. Tecnología para volverlo simple y accesible.</small></div></div></div><div className="team-public team-public-v2"><article className="team-profile"><div className="team-photo"><img src="/team/juan-manuel-iglesias.jpg" alt="Juan Manuel Iglesias en el campo"/><span>CAMPO Y PRODUCCIÓN</span></div><div className="team-profile-copy"><small>COFUNDADOR</small><h3>Juan Manuel Iglesias</h3><strong>Ingeniero Agrónomo</strong><p>Visión agronómica, procesos productivos y necesidades reales del trabajo diario en el campo.</p></div></article><article className="team-profile"><div className="team-photo team-photo-benicio"><img src="/team/benicio-iglesias-plante-v2.jpg" alt="Benicio Iglesias Plante"/><span>PRODUCTO Y TECNOLOGÍA</span></div><div className="team-profile-copy"><small>COFUNDADOR</small><h3>Benicio Iglesias Plante</h3><strong>Desarrollador de la app y web</strong><p>Producto, experiencia de usuario y desarrollo tecnológico de Growr360.</p></div></article></div></section>
...
function RecordTypePicker({onSelect}:{onSelect:(type:string)=>void}){const types=["sowing","spraying","fertilization","harvest","work","monitoring","soil_analysis","other"];return <div className="record-type-picker"><div><span className="eyebrow">NUEVO REGISTRO</span><h3>¿Qué querés registrar?</h3><p>Elegí una opción para continuar. El tipo quedará fijo durante la carga.</p></div><div>{types.map(type=><button className={`record-type-${type}`} type="button" key={type} onClick={()=>onSelect(type)}><RecordTypeIcon type={type}/><span><strong>{recordType(type)}</strong><small>{recordTypeDescription(type)}</small></span><ChevronRight/></button>)}</div></div>}
