...
function PublicLanding({ onLogin, onRegister, onLegal }: { onLogin: () => void; onRegister: () => void; onLegal: (page:"terms"|"privacy"|"faq") => void }) {
  ...
  return <div className="public-site public-site-v3 pastel-landing">
    ...
    <main>
      ...
      <section className="legal-section legal-section-v3 legal-links"><div className="section-intro"><span>INFORMACIÓN CLARA</span><h2>Tu operación, tus datos.</h2><p>Todo lo que cargás en Growr360 sigue siendo tuyo y podés solicitar una copia completa de tu información.</p></div><div className="legal-grid"><button onClick={()=>onLegal("terms")}><FileText/><span><strong>Términos y condiciones</strong><small>Cómo funciona la plataforma.</small></span><ChevronRight/></button><button onClick={()=>onLegal("privacy")}><ShieldCheck/><span><strong>Privacidad y datos</strong><small>Qué protegemos y cómo.</small></span><ChevronRight/></button><button onClick={()=>onLegal("faq")}><CircleUserRound/><span><strong>Preguntas frecuentes</strong><small>Respuestas sobre uso y respaldo.</small></span><ChevronRight/></button></div></section>
    ...
    <footer className="public-footer public-footer-v2"><div className="footer-shell"><div className="footer-brand-block"><Brand/><p>Gestión agrícola inteligente para equipos que necesitan trabajar con información clara, dentro y fuera del campo.</p><span><MapPin/>Chivilcoy, Buenos Aires · Argentina</span><a href="tel:+5492346458558"><Phone/>+54 9 2346 458558</a></div><div className="footer-links"><strong>Growr360</strong><a href="#producto">Plataforma</a><a href="#funciones">Funciones</a><a href="#nosotros">Quiénes somos</a></div><div className="footer-links"><strong>Información</strong><button onClick={()=>onLegal("terms")}>Términos y condiciones</button><button onClick={()=>onLegal("privacy")}>Privacidad</button><button onClick={()=>onLegal("faq")}>Preguntas frecuentes</button></div><div className="footer-downloads"><strong>Growr360 en todos tus dispositivos</strong><p>Trabajá desde la computadora o llevá la gestión al campo.</p><div><button className="store-badge" aria-label="Google Play próximamente"><img src="/google-play-badge.png" alt="Disponible en Google Play"/><small>PRÓXIMAMENTE</small></button><button className="web-access-badge" onClick={onLogin}><Map/><span><small>ACCESO DESDE</small><strong>Aplicación web</strong></span></button></div></div><div className="footer-bottom"><span>© 2026 Growr360. Todos los derechos reservados.</span><span>Hecho en Chivilcoy para el campo argentino.</span></div></div></footer>
  </div>;
}
