...
function RecordListSummary({row}:{row:RecordRow}){
  ...
  if(type==="harvest"){
    ...
    return <div className="record-list-summary harvest-summary"><span><small>Producción</small><strong>{total.toLocaleString("es-AR",{maximumFractionDigits:2})} {unit}</strong></span><span><small>Rendimiento</small><strong>{yieldPerHa.toLocaleString("es-AR",{maximumFractionDigits:2})} {unit}/ha</strong></span></div>;
  }
...
function PlansView({plans,subscription,plots,campaigns,assignments,members,groupName}:{plans:SubscriptionPlan[];subscription:GroupSubscription|null;plots:Plot[];campaigns:Campaign[];assignments:PlotCampaign[];members:Member[];groupName:string}){
  ...
  const planPrice=(code:SubscriptionPlan["code"])=>code==="free"?{amount:"ARS 0",caption:"para siempre"}:code==="pro"?{amount:"ARS 80.000",caption:"por mes"}:{amount:"ARS 150.000",caption:"por mes"};
