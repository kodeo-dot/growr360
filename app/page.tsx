...
function PublicLanding({ onLogin, onRegister }: { onLogin: () => void; onRegister: () => void }) {
  return <div className="public-site">
    ...
    <footer className="public-footer public-footer-v2"><div className="footer-brand-block"><Brand/><p>Gestión agrícola inteligente para equipos que necesitan trabajar con información clara, dentro y fuera del campo.</p><span><MapPin/>Chivilcoy, Buenos Aires · Argentina</span></div><div className="footer-links"><strong>Growr360</strong><a href="#producto">Producto</a><a href="#soluciones">Soluciones</a><a href="#nosotros">Quiénes somos</a></div><div className="footer-links"><strong>Información</strong><a href="#terminos">Términos y condiciones</a><a href="#terminos">Privacidad</a><button onClick={onLogin}>Ingresar a la web</button></div><div className="footer-downloads"><strong>Growr360 en todos tus dispositivos</strong><p>Trabajá desde la computadora o llevá la gestión al campo.</p><div><button className="store-badge" aria-label="Google Play próximamente"><img src="/google-play-badge.png" alt="Disponible en Google Play"/><small>PRÓXIMAMENTE</small></button><button className="web-access-badge" onClick={onLogin}><Map/><span><small>ACCESO DESDE</small><strong>Aplicación web</strong></span></button></div></div><div className="footer-bottom"><span>© 2026 Growr360. Todos los derechos reservados.</span><span>Hecho en Chivilcoy para el campo argentino.</span></div></footer>
  </div>;
}
...
function RealReportsView({ fields, plots, records, crops }: { fields: Field[]; plots: Plot[]; records: RecordRow[]; crops: Crop[] }) {
  ...
  return <div className="page-content"><PageHead title="Reportes" text="El mismo análisis operativo de la app, con filtros combinables."/>
    <div className="report-filter premium-filter"><select value={fieldId} onChange={e => { setFieldId(e.target.value); setPlotId(""); }}><option value="">Todos los campos</option>{fields.map(field => <option key={field.id} value={field.id}>{field.name}</option>)}</select><select value={plotId} onChange={e => setPlotId(e.target.value)}><option value="">Todos los lotes</option>{plots.filter(plot => !fieldId || plot.field_id === fieldId).map(plot => <option key={plot.id} value={plot.id}>{plot.name}</option>)}</select><select value={campaignId} onChange={e => setCampaignId(e.target.value)}><option value="">Todas las campañas</option>{Array.from(new globalThis.Map(records.map(row=>[row.campaign_id,relation(row.campaigns)?.name])).entries()).filter(([id])=>id).map(([id,name])=><option key={id!} value={id!}>{name}</option>)}</select><select value={crop} onChange={e => setCrop(e.target.value)}><option value="">Todos los cultivos</option>{crops.map(item => <option key={item.id} value={item.name}>{item.name}</option>)}</select><select value={type} onChange={e => setType(e.target.value)}><option value="">Todos los registros</option>{Array.from(new Set(records.map(effectiveRecordType))).filter(item=>!["monitoring","soil_analysis"].includes(item)).map(item => <option key={item} value={item}>{recordType(item)}</option>)}</select><select value={contractor} onChange={e => setContractor(e.target.value)}><option value="">Todos los contratistas</option>{contractors.map(item=><option key={item} value={item}>{item}</option>)}</select><button onClick={() => { setFieldId(""); setPlotId(""); setCampaignId(""); setCrop(""); setType(""); setContractor(""); }}><RotateCcw/>Limpiar</button></div>
