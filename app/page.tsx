...
function parseSeverityValue(value:unknown){
  ...
  String(value??"").split("|").forEach(item=>{const separator=item.lastIndexOf(":");if(separator<1)return;const name=item.slice(0,separator).trim();const level=Math.round(number(item.slice(separator+1)));if(name&&level>=1&&level<=5)result[name]=level});
  return result;
}
