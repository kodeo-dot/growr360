  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {!groupId ? <EmptyWorkspace onGroups={() => setGroupBrowserOpen(true)}/> : <>
        ...
        {view === "registros" && <RealRecordsView mode="records" groupId={groupId} userId={session.user.id} records={records} canCreate={hasPermission("create_records")} canDeleteAny={hasPermission("delete_any_records")} canDeleteOwn={hasPermission("delete_own_records")} onCreate={() => setPendingForm("record")} onSaved={()=>void loadGroupData(groupId,true)}/>} 
        {view === "monitoreos" && <RealRecordsView mode="monitoring" groupId={groupId} userId={session.user.id} records={records} canCreate={hasPermission("create_monitoring")} canDeleteAny={hasPermission("delete_any_records")} canDeleteOwn={hasPermission("delete_own_records")} onCreate={() => { setPendingRecord({plotId:"",type:"monitoring"}); setPendingForm("record"); }} onSaved={()=>void loadGroupData(groupId,true)}/>} 
...
function RealRecordsView({ records, canCreate, onCreate, mode = "records", groupId, userId, canDeleteAny, canDeleteOwn, onSaved }: { records: RecordRow[]; canCreate:boolean; onCreate:()=>void; mode?:"records"|"monitoring"; groupId:string; userId:string; canDeleteAny:boolean; canDeleteOwn:boolean; onSaved:()=>void }) {
  ...
  return <div className="page-content records-ot-style records-bulk-v67">
    ...
    <section className="content-card records-directory records-directory-v25">
      ...
      {selectedRows.length>0&&<div className="records-bulkbar-v67"><div><strong>{selectedRows.length} seleccionado{selectedRows.length===1?"":"s"}</strong><button type="button" onClick={()=>setSelectedIds([])}>Limpiar selección</button></div><div><button type="button" disabled={bulkBusy} onClick={()=>void exportSelectedRecords()}>{bulkBusy?<LoaderCircle className="spin"/>:<Download/>}Exportar PDFs</button>{selectedDeletable.length>0&&<button type="button" className="danger-text" disabled={bulkBusy} onClick={()=>void deleteSelectedRecords()}><Trash2/>Eliminar{selectedDeletable.length>1?` (${selectedDeletable.length})`:""}</button>}</div></div>}
...
function WorkOrdersView({groupId,userId,orders,records,fields,plots,campaigns,members,contractors,supplies,plan,canCreate,canEditAny,canEditOwn,canDeleteAny,canDeleteOwn,onSaved}:{groupId:string;userId:string;orders:WorkOrder[];records:RecordRow[];fields:Field[];plots:Plot[];campaigns:Campaign[];members:Member[];contractors:Contractor[];supplies:Supply[];plan:string;canCreate:boolean;canEditAny:boolean;canEditOwn:boolean;canDeleteAny:boolean;canDeleteOwn:boolean;onSaved:()=>void}){
