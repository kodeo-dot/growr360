...
function RealMapView({ fields, plots, records, campaigns, assignments, cropColors, crops, selectedPlot, setSelectedPlot, groupId, userId, canManageLots, onCreateRecord, onSaved }: {
  ...
  return <div className="map-workspace">
    ...
    {satelliteOpen && <aside className="satellite-panel real-satellite"><div className="sat-top"><div><span className="eyebrow">PLANET INSIGHTS</span><strong>Imágenes satelitales</strong></div><button onClick={() => setSatelliteOpen(false)}><X/></button></div>
...
function RecordDetail({ record, onClose }: { record: RecordRow; onClose: () => void }) {
  ...
  const visible=visibleDetails(details);
...
function RecordAttachments({recordId}:{recordId:string}){
  ...
  const[loading,setLoading]=useState(true);
  const[error,setError]=useState("");
  const[selected,setSelected]=useState<ResolvedAttachment|null>(null);
  ...
  return <section className="record-attachments"><div className="attachment-title"><Paperclip/><div><h3>Fotos y archivos</h3><p>{files.length} adjunto{files.length===1?"":"s"} en este registro</p></div></div>{error&&<p className="form-error">{error}</p>}<div className="attachment-grid">{files.map(file=>file.mime_type?.startsWith("image/")?<button type="button" key={file.id} className="attachment-image" onClick={()=>setSelected(file)}><img src={file.url} alt={file.file_name}/><span><Maximize2/>{file.file_name}</span></button>:<a key={file.id} className="attachment-file" href={file.url} target="_blank" rel="noreferrer"><FileText/><span><strong>{file.file_name}</strong><small>{attachmentSize(file.size_bytes)}</small></span><Download/></a>)}</div>{selected&&<div className="photo-viewer" role="dialog" aria-modal="true" aria-label={`Vista de ${selected.file_name}`} onClick={()=>setSelected(null)}><button type="button" className="photo-viewer-close" onClick={()=>setSelected(null)} aria-label="Cerrar"><X/></button><img src={selected.url} alt={selected.file_name} onClick={event=>event.stopPropagation()}/><div><strong>{selected.file_name}</strong><a href={selected.url} target="_blank" rel="noreferrer"><Download/>Abrir original</a></div></div>}</section>;
}
...
function InlineRecordDetail({record}:{record:RecordRow}){
  ...
  const technical=entries.filter(([key])=>!inputs.some(([item])=>item===key)&&!costs.some(([item])=>item===key)&&!gps.some(([item])=>item===key));
  return <div className="inline-record-detail">
    ...
    <div className="inline-detail-groups">
      {inputs.length>0&&<InlineDetailGroup title="Insumos aplicados" entries={inputs} open/>}
...
function RealTeamView({ section, groupId, members, currentRole, canManage, onSection, onSaved }: { section:"members"|"requests"|"invitations"; groupId:string; members: Member[]; currentRole:string; canManage:boolean; onSection:(view:View)=>void; onSaved:()=>void }) {
  ...
  if(section==="members")return <div className="page-content"><PageHead title="Equipo" text="Integrantes, roles y permisos del grupo."/>{canManage&&<div className="team-shortcuts"><button onClick={()=>onSection("solicitudes")}><span><Users/></span><div><strong>Solicitudes de ingreso</strong><small>{loadingRequests?"Actualizando…":requests.length?`${requests.length} pendiente${requests.length===1?"":"s"}`:"No hay solicitudes pendientes"}</small></div><ChevronRight/></button><button onClick={()=>onSection("invitaciones")}><span><Link2/></span><div><strong>Invitaciones por enlace</strong><small>Crear, copiar y revocar accesos directos</small></div><ChevronRight/></button></div>}{message&&<p className="form-error">{message}</p>}<div className="team-section-title"><div><span className="eyebrow">INTEGRANTES ACTIVOS</span><h3>{members.length} miembro{members.length===1?"":"s"}</h3></div></div><div className="team-grid">{members.map(member=>{const profile=relation(member.profiles);const memberName=[profile?.first_name,profile?.last_name].filter(Boolean).join(" ")||profile?.username||"Usuario";return <button className="member-card member-card-button" key={member.user_id} onClick={()=>openMember(member)}><div className="member-avatar">{initials(memberName)}</div><div><h3>{memberName}</h3><p>{roleName(member.role)}</p></div><span className="member-active"><i/>Activo</span><div className="access"><small>Cuenta</small><strong>{profile?.email??"Sin correo visible"}</strong></div><ChevronRight/></button>})}{!members.length&&<EmptyLine text="No hay miembros visibles."/>}</div>{selected&&<div className="record-detail-backdrop"><article className="permission-sheet"><header><div><span className="eyebrow">MIEMBRO DEL EQUIPO</span><h2>{[relation(selected.profiles)?.first_name,relation(selected.profiles)?.last_name].filter(Boolean).join(" ")}</h2><p>{relation(selected.profiles)?.email}</p></div><button className="icon-button" onClick={()=>setSelected(null)}><X/></button></header><label>Rol<select disabled={!canManage||selected.role==="owner"||busy===selected.user_id} value={selected.role} onChange={event=>{void role(selected.user_id,event.target.value);setSelected({...selected,role:event.target.value})}}>{["admin","agronomist","operator","monitor","producer","member"].map(item=><option key={item} value={item}>{roleName(item)}</option>)}</select></label><section><h3>Permisos personalizados</h3><p>Estos cambios tienen prioridad sobre los permisos predeterminados del rol.</p><div className="permission-grid">{permissionCatalog.map(([key,label])=><label key={key}><span>{label}</span><input type="checkbox" disabled={!canManage||selected.role==="owner"} checked={Boolean(draft[key])} onChange={event=>setDraft({...draft,[key]:event.target.checked})}/></label>)}</div></section>{canManage&&selected.role!=="owner"&&<footer><button className="danger-button" onClick={()=>void remove(selected.user_id)}>Quitar del grupo</button><button className="settings-save" onClick={()=>void savePermissions()}><Save/>Guardar permisos</button></footer>}</article></div>}</div>;
...
const DETAIL_LABELS:Record<string,string>={
  ...
  weeds:"Malezas",weed_levels:"Nivel de infestación por maleza",insects:"Insectos",insect_levels:"Nivel por insecto",diseases:"Enfermedades",disease_levels:"Nivel por enfermedad",plant_count:"Conteo de plantas",plant_count_unit:"Unidad del conteo",
...
function SeverityDetail({value}:{value:unknown}){const entries=String(value??"").split("|").map(item=>{const separator=item.lastIndexOf(":");return separator>0?[item.slice(0,separator).trim(),Number(item.slice(separator+1))] as const:null}).filter((item):item is readonly[string,number]=>Boolean(item&&item[0]&&item[1]>=1&&item[1]<=5));if(!entries.length)return <span>Sin niveles informados</span>;return <span className="severity-detail-list">{entries.map(([name,level])=><span key={name} style={{"--severity":severityColor(level)} as React.CSSProperties}><i/>{name}<b>{level}</b></span>)}</span>}
function formatDetailValue(key:string,value:unknown){if(key.endsWith("_levels"))return <SeverityDetail value={value}/>;if(typeof value==="boolean")return value?"Sí":"No";if(key.includes("depth")&&String(value).trim()!=="")return `${number(value as string|number).toLocaleString("es-AR",{maximumFractionDigits:2})} cm`;if(key.endsWith("_at")){const date=new Date(String(value));if(!Number.isNaN(date.getTime()))return new Intl.DateTimeFormat("es-AR",{dateStyle:"medium",timeStyle:"short"}).format(date);}return String(value);}
