...
function WorkOrdersView({groupId,userId,orders,fields,plots,campaigns,members,contractors,supplies,plan,canCreate,canEditAny,canEditOwn,onSaved}:{groupId:string;userId:string;orders:WorkOrder[];fields:Field[];plots:Plot[];campaigns:Campaign[];members:Member[];contractors:Contractor[];supplies:Supply[];plan:string;canCreate:boolean;canEditAny:boolean;canEditOwn:boolean;onSaved:()=>void}){
 ...
 const blank=()=>({title:"",order_type:"sowing" as WorkOrderType,status:"pending",priority:"normal",scheduled_date:new Date().toISOString().slice(0,10),scheduled_end_date:"",campaign_id:campaigns.find(c=>c.status==="active")?.id??campaigns[0]?.id??"",field_id:"",plot_id:"",planned_area:"",assigned_to:"",contractor_id:"",instructions:"",notes:"",allow_member_edits:false,products:[] as {input_id:string;product_name:string;dose:string;dose_unit:string;planned_quantity:string}[]});
 ...
 useEffect(()=>{
  ...
  if(item){setSelected(item);setMode("view");}
 },[orders]);
 ...
 async function save(event:FormEvent){
  event.preventDefault();setMessage("");
  if(!draft.title.trim()){setMessage("Escribí un título para la orden.");return}
  if(!draft.campaign_id){setMessage("Seleccioná una campaña.");return}
  if(draft.plot_id&&!draft.field_id){setMessage("Seleccioná el campo al que pertenece el lote.");return}
  setBusy(true);
  try{
   ...
   if(result.error)throw result.error;
   const id=result.data?.id;if(!id)throw new Error("La orden no devolvió un identificador. Volvé a intentar.");
...
function formatDate(value: string) {
  if (!value) return "—";
  // Supabase puede devolver una fecha simple (YYYY-MM-DD) o un timestamp ISO completo.
  // Si ya viene con hora, no agregamos otro `T12:00:00` porque genera una fecha inválida.
  const raw = String(value).trim();
  const dateOnly = /^\d{4}-\d{2}-\d{2}$/.test(raw);
  const date = new Date(dateOnly ? `${raw}T12:00:00` : raw);
  return Number.isNaN(date.getTime())
    ? raw
    : new Intl.DateTimeFormat("es-AR", { day: "2-digit", month: "short", year: "numeric" }).format(date).replace(/\./g, "");
}
