...
function PublicLanding({ onLogin, onRegister }: { onLogin: () => void; onRegister: () => void }) {
  ...
  const modules=[{label:"Mapa",icon:Map},{label:"Campos y lotes",icon:Sprout},{label:"Registros",icon:FileText},{label:"Monitoreos",icon:Eye},{label:"Campañas",icon:CalendarDays},{label:"Reportes",icon:BarChart3},{label:"Equipo",icon:Users},{label:"Satélite",icon:Satellite}];
  return <div className="public-site public-site-v3">
    <header className="public-header public-header-v3"><div className="public-header-main"><Brand/><nav><a href="#producto">Plataforma</a><a href="#funciones">Funciones</a><a href="#soluciones">Soluciones</a><a href="#nosotros">Quiénes somos</a></nav><div className="public-auth-actions"><button className="public-login" onClick={onLogin}>Ingresar</button><button className="public-cta" onClick={onRegister}>Crear cuenta</button></div></div></header>
    <main>
      <section className="public-hero-v3"><img src="/landing/juan-manuel-field.png" alt="Juan Manuel Iglesias utilizando Growr360 en un lote"/><div className="public-hero-shade"/><div className="public-hero-copy"><span>GESTIÓN AGRÍCOLA CONECTADA</span><h1>Toda tu operación<br/>en una plataforma.</h1><p>Organizá campos, lotes, labores, monitoreos y reportes con información compartida por todo el equipo, desde el campo o la oficina.</p><div><button className="public-cta public-cta-light" onClick={onRegister}>Comenzar gratis <ArrowRight/></button><button className="public-hero-login" onClick={onLogin}>Ingresar a Growr360</button></div></div><div className="public-hero-proof"><i/><span><strong>Información sincronizada</strong><small>Web y app trabajan sobre la misma operación</small></span></div></section>

      <section id="producto" className="module-overview"><div className="section-intro"><span>UNA SOLA PLATAFORMA</span><h2>La gestión diaria, ordenada de punta a punta.</h2><p>Ocho áreas conectadas para que el dato se cargue una vez y sirva en toda la operación.</p></div><div className="module-rail">{modules.map(({label,icon:Icon},index)=><article key={label}><div className={`module-icon module-icon-${index}`}><Icon/></div><strong>{label}</strong></article>)}</div></section>

      <section id="funciones" className="product-stories"><article className="product-story product-story-map"><div className="product-story-copy"><span>MAPA PRODUCTIVO</span><h2>El campo completo, en una sola vista.</h2><p>Visualizá cultivos, prioridades, campañas y monitoreos sobre cada lote. Dibujá polígonos o importá archivos KML/KMZ sin salir de la plataforma.</p><ul><li><Check/>Capas y filtros por campaña</li><li><Check/>Monitoreos geolocalizados</li><li><Check/>Imágenes Planet e índices NDVI</li></ul></div><ProductScreenshot src="/landing/growr-map.png" alt="Mapa productivo de Growr360"/></article><article className="product-story product-story-reports"><ProductScreenshot src="/landing/growr-reports.png" alt="Reporte de avance de siembra de Growr360"/><div className="product-story-copy"><span>REPORTES OPERATIVOS</span><h2>Del registro diario a una decisión clara.</h2><p>Convertí la actividad del equipo en gráficos por campaña, cultivo, campo o lote. Seguí el avance de siembra día por día y evitá duplicar superficie.</p><ul><li><Check/>Superficie acumulada y porcentaje</li><li><Check/>Costos, rendimiento y contratistas</li><li><Check/>Filtros simples con detalle trazable</li></ul></div></article></section>

      <section id="soluciones" className="value-section"><div className="section-intro"><span>HECHO PARA EL TRABAJO REAL</span><h2>Menos tiempo buscando datos.<br/>Más tiempo tomando decisiones.</h2></div><div className="value-grid"><article><ClockIcon/><strong>Trabajo ágil</strong><p>Flujos simples para cargar información mientras la labor sucede.</p></article><article><Users/><strong>Equipo coordinado</strong><p>Roles, permisos y varios grupos sin mezclar responsabilidades.</p></article><article><CloudSun/><strong>Dentro y fuera del campo</strong><p>La misma información disponible en web y aplicación móvil.</p></article><article><ShieldCheck/><strong>Trazabilidad completa</strong><p>Historial, fotos y responsables asociados a cada registro.</p></article></div></section>
      ...
      <section id="terminos" className="legal-section legal-section-v3"><div className="section-intro"><span>INFORMACIÓN LEGAL</span><h2>Transparencia y cuidado de tus datos.</h2></div><div className="legal-grid"><details><summary><FileText/> Términos y condiciones <ChevronDown/></summary><p>Growr360 es una herramienta de apoyo para registrar, organizar y analizar información agrícola. El usuario es responsable por la exactitud de los datos ingresados y por verificar cualquier decisión productiva, técnica o económica antes de ejecutarla.</p></details><details><summary><ShieldCheck/> Privacidad y datos <ChevronDown/></summary><p>Los datos personales, productivos, geográficos y archivos se utilizan para prestar las funciones de la plataforma. No se comercializan datos privados de los usuarios.</p></details><details><summary><LockKeyhole/> Seguridad y ubicación <ChevronDown/></summary><p>Las coordenadas GPS se capturan únicamente al utilizar funciones que las requieren, como monitoreos o trazado de lotes.</p></details></div></section>
    ...
    <footer className="public-footer public-footer-v2"><div className="footer-shell"><div className="footer-brand-block"><Brand/><p>Gestión agrícola inteligente para equipos que necesitan trabajar con información clara, dentro y fuera del campo.</p><span><MapPin/>Chivilcoy, Buenos Aires · Argentina</span></div><div className="footer-links"><strong>Growr360</strong><a href="#producto">Plataforma</a><a href="#funciones">Funciones</a><a href="#nosotros">Quiénes somos</a></div><div className="footer-links"><strong>Información</strong><a href="#terminos">Términos y condiciones</a><a href="#terminos">Privacidad</a><button onClick={onLogin}>Ingresar a la web</button></div><div className="footer-downloads"><strong>Growr360 en todos tus dispositivos</strong><p>Trabajá desde la computadora o llevá la gestión al campo.</p><div><button className="store-badge" aria-label="Google Play próximamente"><img src="/google-play-badge.png" alt="Disponible en Google Play"/><small>PRÓXIMAMENTE</small></button><button className="web-access-badge" onClick={onLogin}><Map/><span><small>ACCESO DESDE</small><strong>Aplicación web</strong></span></button></div></div><div className="footer-bottom"><span>© 2026 Growr360. Todos los derechos reservados.</span><span>Hecho en Chivilcoy para el campo argentino.</span></div></div></footer>
  </div>;
}

function ProductScreenshot({src,alt}:{src:string;alt:string}){return <div className="product-browser"><header><span/><span/><span/></header><div><img src={src} alt={alt}/></div></div>}
