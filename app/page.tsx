...
function ChartDrilldown({label,rows,metric,onClose}:{label:string;rows:RecordRow[];metric:string;onClose:()=>void}){const suffix=metricSuffix(metric);return <section className="chart-drilldown"><header><div><span className="eyebrow">ORIGEN DEL DATO</span><h4>{label}</h4><p>{rows.length} registro{rows.length===1?"":"s"} incluido{rows.length===1?"":"s"}</p></div><button className="icon-button" onClick={onClose}><X/></button></header><div>{rows.sort((a,b)=>b.record_date.localeCompare(a.record_date)).map(row=><article key={row.id}><div><strong>{recordType(effectiveRecordType(row))}</strong><small>{relation(row.fields)?.name||"Campo"} · {relation(row.plots)?.name||"Sin lote"}</small></div><time>{formatDate(row.record_date)}</time><b>{chartValue(row,metric).toLocaleString("es-AR",{maximumFractionDigits:2})}{suffix}</b></article>)}</div></section>}
