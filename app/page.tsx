...
import {
  ...
  Copy, Link2, Mail, Smartphone, UserPlus, Paperclip, Download, Maximize2, Minimize2, FileUp, History, GripVertical, ArrowUp, ArrowDown, Building2, Trash2
} from "lucide-react";
...
type RecordRow = {
  ...
  contractor?: string | null; machinery_text?: string | null; observations?: string | null; created_at?: string | null; created_by?: string | null;
...
function AuthScreen({ client, inviteToken = "" }: { client: SupabaseClient; inviteToken?:string }) {
  ...
  async function register(event: FormEvent) {
    ...
    const { data, error } = await client.auth.signUp({
      ...
      options: { data: { first_name: profileFirst, last_name: profileLast, username: profileUsername } }
    });
    setBusy(false);
...
function AuthenticatedApp({ session }: { session: Session }) {
  ...
  const canManageLots = hasPermission("manage_lots");


  useEffect(() => {
    if (!activeMembership) return;
    const restricted = (["mapa","campos"].includes(view) && !hasPermission("view_fields")) || (["registros","monitoreos","napas","ordenes","reportes"].includes(view) && !hasPermission("view_records"));
    if (restricted) setView("mas");
  }, [activeMembership, view, hasPermission]);
  ...
  const canOpenView = (nextView: View) => {
    if (["mapa","campos"].includes(nextView)) return hasPermission("view_fields");
    if (["registros","monitoreos","napas","ordenes","reportes"].includes(nextView)) return hasPermission("view_records");
    return true;
  };
  const openView = (nextView: View) => {
    if (!canOpenView(nextView)) { setError("No tenés permiso para acceder a esta sección."); return; }
    setView(nextView); setSidebarOpen(false); setGroupBrowserOpen(false);
  };
  const sidebarItem = (id: View, label: string, Icon: typeof Map, badge?: number) => canOpenView(id) ? <button key={id} className={`nav-subitem ${view === id ? "active" : ""}`} onClick={() => openView(id)}><Icon/><span>{label}</span>{badge ? <em>{badge}</em> : null}</button> : null;
  const groupAdmin = activeMembership?.role === "owner" || hasPermission("manage_members");
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
      ...
      <nav className="sidebar-navigation">
        {hasPermission("view_fields") && <button className={`nav-direct ${view === "mapa" ? "active" : ""}`} onClick={() => openView("mapa")}><Map/><span>Mapa</span></button>}
    ...
    <main>
      ...
      {!groupId ? <EmptyWorkspace onGroups={() => setGroupBrowserOpen(true)}/> : <>
        {view === "mapa" && hasPermission("view_fields") && <RealMapView fields={fields} plots={plots} records={hasPermission("view_records")?records:[]} campaigns={campaigns} assignments={assignments} cropColors={cropColors} crops={crops} settings={settings} onSettingsChange={setSettings} selectedPlot={selectedPlot} setSelectedPlot={plot => setSelectedPlotId(plot?.id ?? null)} groupId={groupId} userId={session.user.id} accessToken={session.access_token} plan={activePlan} canManageLots={canManageLots} canViewSatellite={hasPermission("view_satellite")} canViewNdvi={hasPermission("view_ndvi")} canCreateRecords={hasPermission("create_records")} canCreateMonitoring={hasPermission("create_monitoring")} planLotBlocked={planLotLimitReached||planHectareLimitReached} planBlockReason={planBlockReason} planHectaresUsed={planHectaresUsed} planHectareLimit={planHectareLimit} onCreateRecord={(plot,type) => { setPendingRecord({ plotId: plot.id, type }); setPendingForm("record"); setView("registros"); }} onSaved={() => void loadGroupData(groupId, true)}/>}
        {view === "campos" && hasPermission("view_fields") && <RealFieldsView fields={fields} plots={resolvePlotCrops(plots, records, assignments, cropColors, crops)} canCreate={hasPermission("manage_fields") && !planFieldLimitReached} onCreate={() => { const reason=planBlockReason("field"); if(reason){setError(reason);return;} setPendingForm("field"); }} onOpenPlot={plot => { setSelectedPlotId(plot.id); setView("mapa"); }}/>} 
        ...
        {view === "registros" && hasPermission("view_records") && <RealRecordsView mode="records" groupId={groupId} userId={session.user.id} records={records} canCreate={hasPermission("create_records")} canDeleteAny={hasPermission("delete_any_records")} canDeleteOwn={hasPermission("delete_own_records")} canExport={hasPermission("export_reports")} onCreate={() => setPendingForm("record")} onSaved={()=>void loadGroupData(groupId,true)}/>} 
        {view === "monitoreos" && hasPermission("view_records") && <RealRecordsView mode="monitoring" groupId={groupId} userId={session.user.id} records={records} canCreate={hasPermission("create_monitoring")} canDeleteAny={hasPermission("delete_any_records")} canDeleteOwn={hasPermission("delete_own_records")} canExport={hasPermission("export_reports")} onCreate={() => { setPendingRecord({plotId:"",type:"monitoring"}); setPendingForm("record"); }} onSaved={()=>void loadGroupData(groupId,true)}/>} 
        {view === "napas" && hasPermission("view_records") && <NapaView records={records} canCreate={hasPermission("create_records")} onCreate={() => { setPendingRecord({plotId:"",type:"napa"}); setPendingForm("record"); }}/>} 
        ...
        {view === "ordenes" && hasPermission("view_records") && <WorkOrdersView groupId={groupId} userId={session.user.id} orders={workOrders} records={records} fields={fields} plots={plots} campaigns={campaigns} members={members} contractors={contractors} supplies={supplies} plan={activePlan} canCreate={hasPermission("create_records")} canEditAny={hasPermission("edit_records")} canEditOwn={hasPermission("edit_records")} canDeleteAny={hasPermission("delete_any_records")} canDeleteOwn={hasPermission("delete_own_records")} canExport={hasPermission("export_reports")} onSaved={()=>void loadGroupData(groupId,true)}/>} 
        {view === "reportes" && hasPermission("view_records") && <RealReportsView fields={fields} plots={resolvePlotCrops(plots, records, assignments, cropColors, crops)} records={records} crops={crops} campaigns={campaigns}/>}
        {view === "equipo" && <RealTeamView section="members" groupId={groupId} members={members} fields={fields} plots={plots} currentRole={activeMembership?.role ?? "producer"} currentUserId={session.user.id} canManage={hasPermission("manage_members")} memberLimit={currentPlan?.max_users??null} onSection={setView} onSaved={() => void loadGroupData(groupId, true)}/>}
        ...
        {view === "mas" && <MoreView canManageGroup={activeMembership?.role === "owner" || hasPermission("manage_members")} onOpenTeam={()=>setView("equipo")} onOpenSettings={()=>setView("configuracion")} onOpenGroupSettings={()=>setView("grupo")} onOpenPlans={()=>setView("planes")}/>}
        ...
        {view === "configuracion" && <RealSettingsView mode="personal" groupId={groupId} userId={session.user.id} settings={settings} group={group} canManageGroup={activeMembership?.role === "owner" || hasPermission("manage_members")} onSaved={setSettings} onGroupSaved={() => void loadWorkspace()}/>} 
        {view === "grupo" && <RealSettingsView mode="group" groupId={groupId} userId={session.user.id} settings={settings} group={group} canManageGroup={activeMembership?.role === "owner" || hasPermission("manage_members")} onSaved={setSettings} onGroupSaved={() => void loadWorkspace()}/>} 
...
function RealMapView({ fields, plots, records, campaigns, assignments, cropColors, crops, settings, onSettingsChange, selectedPlot, setSelectedPlot, groupId, userId, accessToken, plan, canManageLots, canViewSatellite, canViewNdvi, canCreateRecords, canCreateMonitoring, planLotBlocked, planBlockReason, planHectaresUsed, planHectareLimit, onCreateRecord, onSaved }: {
  ...
  groupId: string; userId: string; accessToken:string; plan: string; canManageLots: boolean; canViewSatellite:boolean; canViewNdvi:boolean; canCreateRecords:boolean; canCreateMonitoring:boolean; planLotBlocked:boolean; planBlockReason:(kind:"field"|"lot",extraArea?:number)=>string; planHectaresUsed:number; planHectareLimit:number|null; onSaved: () => void;
}) {
  ...
  const satelliteFreeIndices = ["RGB", "NDVI", "NDVI_3Z", "NDVI_CONTRASTED"];
  const satelliteProIndices = ["RGB", "NDVI", "NDVI_CONTINUOUS", "NDVI_3Z", "NDVI_5Z", "NDVI_CONTRASTED", "FALSE_COLOR", "NDRE", "SAVI", "EVI"];
  const satelliteIndices = !canViewNdvi ? ["RGB"] : (satelliteAdvanced ? satelliteProIndices : satelliteFreeIndices);
  const satelliteRequestHeaders = { "content-type":"application/json", authorization:`Bearer ${accessToken}` };
  ...
  useEffect(() => {
    if (!canViewSatellite && satelliteOpen) setSatelliteOpen(false);
    if (!canViewNdvi && satelliteIndex !== "RGB") setSatelliteIndex("RGB");
    if (!satelliteAdvanced && !satelliteFreeIndices.includes(satelliteIndex)) {
      setSatelliteIndex("NDVI");
      setSatelliteZoneStats(null);
    }
  }, [satelliteAdvanced, satelliteIndex, canViewSatellite, canViewNdvi, satelliteOpen]);
  ...
  async function loadSatelliteScenes(plotId: string) {
    if (!canViewSatellite) { setSatelliteError("No tenés permiso para ver imágenes satelitales."); setSatelliteOpen(false); return; }
    ...
    try {
      const response = await fetch("/api/satellite/scenes", { method: "POST", headers: satelliteRequestHeaders, body: JSON.stringify({ groupId, geometry: geometry(target.geometry_json) }) });
  ...
  async function openSatellite() {
    if (!canViewSatellite) { setSatelliteError("No tenés permiso para ver imágenes satelitales."); return; }
  ...
  async function loadSatellitePreviews(target: MapPlot, scenes: SatelliteScene[], index: string) {
    ...
    const entries = await Promise.all(scenes.map(async scene => {
      try { const response = await fetch("/api/satellite/image", { method: "POST", headers: satelliteRequestHeaders, body: JSON.stringify({ groupId, geometry: feature, date: scene.date, index: safeIndex, thumbnail: true }) }); if (!response.ok) return [scene.id, ""] as const; return [scene.id, URL.createObjectURL(await response.blob())] as const; } catch { return [scene.id, ""] as const; }
    }));
    setSatellitePreviews(Object.fromEntries(entries));
  }
  async function loadNdviZones(feature: GeoFeature, date: string, zoneCount: 3 | 5) {
    const response = await fetch("/api/satellite/zones", { method: "POST", headers: satelliteRequestHeaders, body: JSON.stringify({ groupId, geometry: feature, date, zoneCount }) });
    const payload = await response.json();
  ...
  async function showSatellite(scene = satelliteScene, index = satelliteIndex) {
    if (!canViewSatellite) { setSatelliteError("No tenés permiso para ver imágenes satelitales."); return; }
    if (index !== "RGB" && !canViewNdvi) { setSatelliteError("No tenés permiso para analizar índices satelitales."); return; }
    ...
    try {
      ...
      const response = await fetch("/api/satellite/image", { method: "POST", headers: satelliteRequestHeaders, body: JSON.stringify({ groupId, geometry: feature, date: scene.date, index: safeIndex, thresholds }) });
  ...
  return <div className="map-workspace">
    ...
    {!drawing && !draft && <div className="map-toolbar">
      ...
      {canViewSatellite&&<button onClick={() => satelliteOpen ? setSatelliteOpen(false) : void openSatellite()} className={satelliteOpen ? "selected" : ""} title="Imágenes satelitales de Planet Insights"><Satellite/><span>Planet</span></button>}
...
function RealPlotPanel({ plot, fieldName, records, onRecord, onNewRecord, onMonitoring, onSatellite, onClose }: { plot: Plot; fieldName: string; records: RecordRow[]; onRecord: (record: RecordRow) => void; onNewRecord?: () => void; onMonitoring?: () => void; onSatellite?: () => void; onClose: () => void }) {
  ...
  return <aside className="lot-panel operational-panel plot-summary-panel"><div className="panel-handle"/><header className="plot-summary-head"><div className="plot-summary-title"><span className="plot-summary-kicker">{fieldName}</span><h2>{plot.name}</h2><div className="plot-summary-crop"><i style={{background:plot.cropColor||"#77847e"}}/><span>{summary.lastCrop||plot.cropName||"Sin cultivo registrado"}</span></div></div><button className="icon-button" onClick={onClose} aria-label="Cerrar lote"><X/></button></header>
    ...
    <div className="quick-actions plot-actions plot-summary-actions">{onSatellite&&<button onClick={onSatellite}><Satellite/><span>Satélite</span></button>}{onNewRecord&&<button onClick={onNewRecord}><Plus/><span>Nuevo registro</span></button>}{onMonitoring&&<button onClick={onMonitoring}><Activity/><span>Monitorear</span></button>}</div>
...
function pdfLatin1(value:string){return value.normalize("NFC").replace(/[\u2018\u2019]/g,"'").replace(/[\u201C\u201D]/g,'"').replace(/[\u2013\u2014]/g,"-").replace(/[^\x00-\xFF]/g,"?")}
function pdfEscape(value:string){return pdfLatin1(value).replace(/\\/g,"\\\\").replace(/\(/g,"\\(").replace(/\)/g,"\\)")}
function wrapPdfLine(value:string,max=72){const text=String(value??"").replace(/\s+/g," ").trim();if(!text)return[""];const words=text.split(" ");const lines:string[]=[];let line="";for(const word of words){const next=line?`${line} ${word}`:word;if(next.length>max&&line){lines.push(line);line=word}else line=next}if(line)lines.push(line);return lines}
function pdfText(x:number,y:number,text:string,size=10,bold=false,color="0.12 0.16 0.13"){return `${color} rg\nBT\n/F${bold?2:1} ${size} Tf\n${x} ${y} Td\n(${pdfEscape(text)}) Tj\nET`}
type PdfPhoto={name:string;data:Uint8Array;width:number;height:number};
function bytesToLatin1(bytes:Uint8Array){let out="";const chunk=0x8000;for(let i=0;i<bytes.length;i+=chunk)out+=String.fromCharCode(...bytes.subarray(i,i+chunk));return out}
async function imageBlobToPdfPhoto(blob:Blob,name:string):Promise<PdfPhoto|null>{try{const url=URL.createObjectURL(blob);const image=await new Promise<HTMLImageElement>((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>reject(new Error("No se pudo procesar la foto"));img.src=url});const max=1600;const scale=Math.min(1,max/Math.max(image.naturalWidth||image.width,image.naturalHeight||image.height));const width=Math.max(1,Math.round((image.naturalWidth||image.width)*scale)),height=Math.max(1,Math.round((image.naturalHeight||image.height)*scale));const canvas=document.createElement("canvas");canvas.width=width;canvas.height=height;const ctx=canvas.getContext("2d");if(!ctx){URL.revokeObjectURL(url);return null}ctx.fillStyle="#fff";ctx.fillRect(0,0,width,height);ctx.drawImage(image,0,0,width,height);URL.revokeObjectURL(url);const jpeg=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,"image/jpeg",0.84));if(!jpeg)return null;return{name,data:new Uint8Array(await jpeg.arrayBuffer()),width,height}}catch{return null}}
async function recordPdfPhotos(recordId:string):Promise<PdfPhoto[]>{const result=await supabase.from("attachments").select("file_name,storage_path,mime_type").eq("record_id",recordId).is("deleted_at",null).order("created_at",{ascending:true});if(result.error)return[];const rows=(result.data??[]).filter(file=>String(file.mime_type??"").startsWith("image/"));const photos=await Promise.all(rows.map(async file=>{const signed=await supabase.storage.from("record-attachments").createSignedUrl(String(file.storage_path),60*15);if(!signed.data?.signedUrl)return null;try{const response=await fetch(signed.data.signedUrl);if(!response.ok)return null;return await imageBlobToPdfPhoto(await response.blob(),String(file.file_name||"Foto"))}catch{return null}}));return photos.filter((photo):photo is PdfPhoto=>Boolean(photo))}
function buildSimplePdf(title:string,rawLines:string[],photos:PdfPhoto[]=[]){
  const pageW=595,pageH=842,margin=42,contentW=pageW-margin*2;
  ...
  const pages:string[][]=[];let commands:string[]=[];let y=0;let pageNo=0;
  const green="0.11 0.31 0.18",green2="0.20 0.43 0.27",ink="0.10 0.14 0.11",muted="0.42 0.48 0.43",line="0.87 0.90 0.87",soft="0.95 0.97 0.95",soft2="0.91 0.95 0.92";
  const rect=(x:number,y0:number,w:number,h:number,fill:string,stroke?:string)=>`${fill} rg\n${stroke?`${stroke} RG\n`:""}${x} ${y0} ${w} ${h} re ${stroke?"B":"f"}`;
  const header=()=>{pageNo++;commands=["q",rect(0,0,pageW,pageH,"1 1 1"),rect(0,0,10,pageH,green),rect(10,760,pageW-10,82,"0.985 0.99 0.985"),"Q",pdfText(42,812,"GROWR360",15,true,green),pdfText(42,791,"GESTIÓN AGRÍCOLA",7.4,true,green2),pdfText(500,811,"DOCUMENTO",7,true,muted),pdfText(500,793,new Intl.DateTimeFormat("es-AR").format(new Date()),8.5,true,ink)];const cleanTitle=wrapPdfLine(title,58);cleanTitle.slice(0,2).forEach((line,index)=>commands.push(pdfText(42,754-index*18,line,index===0?17:13,true,ink)));commands.push(`${line} RG`,`42 724 m 553 724 l S`);y=694;};
  const footer=()=>{commands.push(`${line} RG`,`42 42 m 553 42 l S`,pdfText(42,24,"Growr360 · Gestión agrícola",7.4,true,green),pdfText(205,24,"growr360.com",7.4,false,muted),pdfText(532,24,`Pág. ${pageNo}`,7.4,true,muted));};
  const newPage=()=>{if(commands.length){footer();pages.push(commands)}header()};header();
  const need=(h:number)=>{if(y-h<64)newPage()};
  const section=(label:string)=>{need(48);y-=8;commands.push(rect(margin,y-31,contentW,31,soft2),pdfText(margin+12,y-20,label,8.4,true,green));y-=43;};
  const row=(label:string,value:string)=>{const wrapped=wrapPdfLine(value||"—",52);const h=Math.max(40,28+(wrapped.length-1)*12);need(h+5);commands.push(rect(margin,y-h+3,contentW,h,"0.992 0.994 0.992",line),pdfText(margin+12,y-17,label.toUpperCase(),7.1,true,muted));wrapped.forEach((lineText,j)=>commands.push(pdfText(margin+176,y-17-j*12,lineText,9.5,j===0,ink)));y-=h+7;};
  for(let i=0;i<rawLines.length;i++){
    const raw=String(rawLines[i]??"").trim();
    if(!raw){y-=8;continue}
    const isSection=/^[A-ZÁÉÍÓÚÜÑ0-9 Y/·-]{4,}$/.test(raw)&&!raw.includes(":");
    if(isSection){section(raw);continue}
    const colon=raw.indexOf(":");
    if(colon>0&&colon<42){row(raw.slice(0,colon).trim(),raw.slice(colon+1).trim());continue}
    const wrapped=wrapPdfLine(raw,76);const h=wrapped.length*13+12;need(h);commands.push(rect(margin,y-h+4,contentW,h,"1 1 1",line));wrapped.forEach((lineText,j)=>commands.push(pdfText(margin+12,y-15-j*13,lineText,9,false,ink)));y-=h+7;
  }
  footer();pages.push(commands);
  const pageIds:number[]=[];
  pages.forEach(bodyCommands=>{const body=bodyCommands.join("\n");const contentId=add(`<< /Length ${pdfLatin1(body).length} >>\nstream\n${body}\nendstream`);const pageId=add(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${fontId} 0 R /F2 ${fontBoldId} 0 R >> >> /Contents ${contentId} 0 R >>`);pageIds.push(pageId)});
  photos.forEach((photo,index)=>{const imageBody=bytesToLatin1(photo.data);const imageId=add(`<< /Type /XObject /Subtype /Image /Width ${photo.width} /Height ${photo.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${photo.data.length} >>\nstream\n${imageBody}\nendstream`);const maxW=485,maxH=560;const scale=Math.min(maxW/photo.width,maxH/photo.height);const drawW=photo.width*scale,drawH=photo.height*scale;const x=(595-drawW)/2,yImg=130+(560-drawH)/2;const caption=wrapPdfLine(photo.name||`Foto ${index+1}`,62).slice(0,2);const body=["q",rect(0,0,pageW,pageH,"1 1 1"),rect(0,0,10,pageH,green),rect(10,760,pageW-10,82,"0.985 0.99 0.985"),"Q",pdfText(42,812,"GROWR360",15,true,green),pdfText(42,791,"REGISTRO FOTOGRÁFICO",7.4,true,green2),pdfText(42,748,`Fotografía ${index+1} de ${photos.length}`,17,true,ink),`${line} RG`,`42 724 m 553 724 l S`,rect(42,112,511,584,"0.985 0.988 0.985",line),"q",`${drawW.toFixed(2)} 0 0 ${drawH.toFixed(2)} ${x.toFixed(2)} ${yImg.toFixed(2)} cm`,`/Im1 Do`,`Q`,...caption.map((lineText,j)=>pdfText(54,92-j*12,lineText,9,j===0,ink)),`${line} RG`,`42 42 m 553 42 l S`,pdfText(42,24,"Growr360 · Gestión agrícola",7.4,true,green),pdfText(205,24,"growr360.com",7.4,false,muted),pdfText(532,24,`Pág. ${pages.length+index+1}`,7.4,true,muted)].join("\n");const contentId=add(`<< /Length ${pdfLatin1(body).length} >>\nstream\n${body}\nendstream`);const pageId=add(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${fontId} 0 R /F2 ${fontBoldId} 0 R >> /XObject << /Im1 ${imageId} 0 R >> >> /Contents ${contentId} 0 R >>`);pageIds.push(pageId)});
  ...
  let pdf="%PDF-1.4\n%âãÏÓ\n";const offsets=[0];objects.forEach((obj,index)=>{offsets[index+1]=pdfLatin1(pdf).length;pdf+=`${index+1} 0 obj\n${obj}\nendobj\n`});const xref=pdfLatin1(pdf).length;pdf+=`xref\n0 ${objects.length+1}\n0000000000 65535 f \n`;for(let i=1;i<=objects.length;i++)pdf+=`${String(offsets[i]).padStart(10,"0")} 00000 n \n`;pdf+=`trailer\n<< /Size ${objects.length+1} /Root ${catalogId} 0 R >>\nstartxref\n${xref}\n%%EOF`;
  return Uint8Array.from(pdfLatin1(pdf),char=>char.charCodeAt(0)&255);
}
function safeExportName(value:string,fallback="archivo"){return value.normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9_-]+/g,"-").replace(/^-+|-+$/g,"").slice(0,70)||fallback}
function recordPdfLines(row:RecordRow){
  const details=recordData(row),area=number(row.worked_area),inputs=recordInputDetails(details,area);
  const hidden=(key:string)=>key==="record_kind"||key==="observations"||key==="input_count"||key.startsWith("input_")||key.startsWith("source_")||key.startsWith("work_order_")||key.endsWith("_id");
  const lines=["RESUMEN DEL REGISTRO",`Tipo: ${recordType(effectiveRecordType(row))}`,`Fecha: ${row.record_date?formatDate(row.record_date):"—"}`,`Campo: ${relation(row.fields)?.name??"—"}`,`Lote: ${relation(row.plots)?.name??"—"}`,`Campaña: ${relation(row.campaigns)?.name??"—"}`,`Cultivo: ${recordCrop(row)||"—"}`,`Superficie trabajada: ${row.worked_area!=null?`${area.toLocaleString("es-AR",{maximumFractionDigits:2})} ha`:"—"}`];
  if(inputs.length){lines.push("","INSUMOS Y DOSIS");inputs.forEach(input=>{const unit=input.unit||"u/ha";const totalUnit=unit.replace(/\/ha$/i,"")||"u";lines.push(`Insumo: ${input.name||`Insumo ${input.index+1}`}`,`Dosis: ${input.dose.toLocaleString("es-AR",{maximumFractionDigits:2})} ${unit}`,`Cantidad total: ${input.quantity.toLocaleString("es-AR",{maximumFractionDigits:2})} ${totalUnit}`)})}
  if(row.contractor||row.machinery_text){lines.push("","EJECUCIÓN");if(row.contractor)lines.push(`Contratista / responsable: ${row.contractor}`);if(row.machinery_text)lines.push(`Maquinaria: ${row.machinery_text}`)}
  const technical=Object.entries(details).filter(([key,value])=>!hidden(key)&&value!==null&&value!==undefined&&String(value).trim()!=="");
  if(technical.length){lines.push("","DATOS TÉCNICOS");technical.forEach(([key,value])=>{let rendered:string;if(typeof value==="boolean")rendered=value?"Sí":"No";else if(value&&typeof value==="object")rendered=translatedDetailText(JSON.stringify(value));else rendered=translatedDetailText(value);lines.push(`${detailLabel(key)}: ${rendered}`)})}
  if(row.observations){lines.push("","OBSERVACIONES",row.observations)}
  return lines;
}

function RealRecordsView({ records, canCreate, onCreate, mode = "records", groupId, userId, canDeleteAny, canDeleteOwn, canExport, onSaved }: { records: RecordRow[]; canCreate:boolean; onCreate:()=>void; mode?:"records"|"monitoring"; groupId:string; userId:string; canDeleteAny:boolean; canDeleteOwn:boolean; canExport:boolean; onSaved:()=>void }) {
  ...
  const selectedRows=baseRows.filter(row=>selectedIds.includes(row.id));
  const allVisibleSelected=visible.length>0&&visible.every(row=>selectedIds.includes(row.id));
  const selectedDeletable=(canDeleteAny||canDeleteOwn)?selectedRows:[];
  const toggleSelected=(id:string)=>setSelectedIds(current=>current.includes(id)?current.filter(item=>item!==id):[...current,id]);
  const toggleAllVisible=()=>setSelectedIds(current=>allVisibleSelected?current.filter(id=>!visible.some(row=>row.id===id)):Array.from(new Set([...current,...visible.map(row=>row.id)])));
  ...
  async function exportSelectedRecords(){if(!canExport||!selectedRows.length){setBulkMessage("No tenés permiso para exportar reportes.");return;}setBulkBusy(true);setBulkMessage("");try{const zip=new JSZip();const folder=zip.folder(monitoring?"monitoreos":"registros");for(let index=0;index<selectedRows.length;index++){const row=selectedRows[index];const title=`${recordType(effectiveRecordType(row))} · ${relation(row.fields)?.name??"Campo"} · ${relation(row.plots)?.name??"Lote"}`;const photos=await recordPdfPhotos(row.id);folder?.file(`${String(index+1).padStart(2,"0")}-${safeExportName(title,"registro")}.pdf`,buildSimplePdf(title,recordPdfLines(row),photos))}const blob=await zip.generateAsync({type:"blob",compression:"DEFLATE"});const url=URL.createObjectURL(blob);const anchor=document.createElement("a");anchor.href=url;anchor.download=`growr360-${monitoring?"monitoreos":"registros"}-${new Date().toISOString().slice(0,10)}.zip`;anchor.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch(error){setBulkMessage(spanishError(error))}finally{setBulkBusy(false)}}
  async function deleteSelectedRecords(){if(!selectedRows.length||(!canDeleteAny&&!canDeleteOwn))return;const text=`¿Eliminar ${selectedRows.length} ${monitoring?"monitoreo":"registro"}${selectedRows.length===1?"":"s"}? Solo se eliminarán los que tu permiso permita.`;if(!window.confirm(text))return;setBulkBusy(true);setBulkMessage("");try{const result=await supabase.rpc("delete_records",{p_record_ids:selectedRows.map(row=>row.id)});if(result.error)throw result.error;const deleted=Number(result.data??0);if(deleted<selectedRows.length)setBulkMessage(`${deleted} eliminado${deleted===1?"":"s"}. ${selectedRows.length-deleted} no se pudieron eliminar por permisos o porque ya no existían.`);setSelectedIds([]);onSaved()}catch(error){setBulkMessage(spanishError(error))}finally{setBulkBusy(false)}}
  return <div className="page-content records-ot-style records-bulk-v67">
    ...
    <section className="content-card records-directory records-directory-v25">
      ...
      <div className="records-directory-meta records-directory-meta-v67"><div><button type="button" className={`record-check-v67 ${allVisibleSelected?"checked":""}`} onClick={toggleAllVisible} aria-label="Seleccionar registros visibles">{allVisibleSelected&&<Check/>}</button><span>{visible.length} {monitoring?"monitoreo":"registro"}{visible.length===1?"":"s"}</span></div>{hasFilters&&<button type="button" onClick={clearFilters}>Limpiar filtros</button>}</div>
      {selectedRows.length>0&&<div className="records-bulkbar-v67"><div><strong>{selectedRows.length} seleccionado{selectedRows.length===1?"":"s"}</strong><button type="button" onClick={()=>setSelectedIds([])}>Limpiar selección</button></div><div>{canExport&&<button type="button" disabled={bulkBusy} onClick={()=>void exportSelectedRecords()}>{bulkBusy?<LoaderCircle className="spin"/>:<Download/>}Exportar PDFs</button>}{selectedDeletable.length>0&&<button type="button" className="danger-text" disabled={bulkBusy} onClick={()=>void deleteSelectedRecords()}><Trash2/>Eliminar{selectedDeletable.length>1?` (${selectedDeletable.length})`:""}</button>}</div></div>}
      {bulkMessage&&<p className="form-error records-bulk-error-v67">{bulkMessage}</p>}
      <div className="record-list records-ot-list records-select-list-v67">{visible.map(row => <div className={`record-select-row-v67 ${selectedIds.includes(row.id)?"selected":""}`} key={row.id}>
        <button type="button" className={`record-check-v67 ${selectedIds.includes(row.id)?"checked":""}`} onClick={()=>toggleSelected(row.id)} aria-label={`Seleccionar ${recordType(effectiveRecordType(row))}`}>{selectedIds.includes(row.id)&&<Check/>}</button>
        <button type="button" className="record-card" onClick={()=>setSelected(row)}>
      ...
      </div>)}{!visible.length && <EmptyLine text={monitoring ? "No hay monitoreos con estos filtros." : "No hay registros con estos filtros."}/>}</div>
    </section>
...
function WorkOrdersView({groupId,userId,orders,records,fields,plots,campaigns,members,contractors,supplies,plan,canCreate,canEditAny,canEditOwn,canDeleteAny,canDeleteOwn,canExport,onSaved}:{groupId:string;userId:string;orders:WorkOrder[];records:RecordRow[];fields:Field[];plots:Plot[];campaigns:Campaign[];members:Member[];contractors:Contractor[];supplies:Supply[];plan:string;canCreate:boolean;canEditAny:boolean;canEditOwn:boolean;canDeleteAny:boolean;canDeleteOwn:boolean;canExport:boolean;onSaved:()=>void}){
 ...
 const [status,setStatus]=useState("pending");const [query,setQuery]=useState("");const [selected,setSelected]=useState<WorkOrder|null>(null);const [mode,setMode]=useState<"view"|"form"|"complete"|null>(null);const [busy,setBusy]=useState(false);const [message,setMessage]=useState("");const [formStep,setFormStep]=useState(0);const [collapsedProducts,setCollapsedProducts]=useState<Record<number,boolean>>({});const [selectedIds,setSelectedIds]=useState<string[]>([]);const [bulkBusy,setBulkBusy]=useState(false);
 ...
 const normalizedStatus=(value:WorkOrder["status"])=>value==="completed"||value==="cancelled"?value:"pending" as const;
 const filtered=[...orders].sort((a,b)=>String(b.created_at??"").localeCompare(String(a.created_at??""))).filter(item=>{const itemStatus=normalizedStatus(item.status);if(status!=="all"&&itemStatus!==status)return false;const lotNames=(item.work_order_plots??[]).map(row=>row.plots?.name??"").join(" ");return `${item.title} ${item.fields?.name??""} ${item.plots?.name??""} ${lotNames}`.toLowerCase().includes(query.toLowerCase())});
 ...
 const stats={pending:orders.filter(o=>normalizedStatus(o.status)==="pending").length,overdue:orders.filter(o=>normalizedStatus(o.status)==="pending"&&Boolean(o.scheduled_date)&&String(o.scheduled_date).slice(0,10)<today).length,done:orders.filter(o=>o.status==="completed").length,cancelled:orders.filter(o=>o.status==="cancelled").length};
 const mayEdit=(item:WorkOrder)=>canEditAny||item.created_by===userId||(Boolean(item.allow_member_edits)&&canEditOwn);
 const mayDelete=(item:WorkOrder)=>canDeleteAny||(item.created_by===userId&&canDeleteOwn);
 const selectedOrders=orders.filter(item=>selectedIds.includes(item.id));
 const deletableSelected=selectedOrders.filter(mayDelete);
 const allFilteredSelected=filtered.length>0&&filtered.every(item=>selectedIds.includes(item.id));
 useEffect(()=>{setSelectedIds(current=>current.filter(id=>orders.some(item=>item.id===id)))},[orders]);
 const toggleSelected=(id:string)=>setSelectedIds(current=>current.includes(id)?current.filter(item=>item!==id):[...current,id]);
 const toggleAllFiltered=()=>setSelectedIds(current=>{const visible=filtered.map(item=>item.id);if(visible.length&&visible.every(id=>current.includes(id)))return current.filter(id=>!visible.includes(id));return Array.from(new Set([...current,...visible]))});
 const safeFileName=(value:string)=>value.normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9_-]+/g,"-").replace(/^-+|-+$/g,"").slice(0,70)||"orden";
 const csvCell=(value:unknown)=>`"${String(value??"").replace(/"/g,'""')}"`;
 function orderPdfLines(item:WorkOrder){const responsable=[item.profiles?.first_name,item.profiles?.last_name].filter(Boolean).join(" ")||item.profiles?.username||"—";const prioridad=item.priority==="urgent"?"Urgente":item.priority==="high"?"Alta":item.priority==="low"?"Baja":"Normal";const lines=["RESUMEN DE LA ORDEN",`Estado: ${statusLabel(item.status)}`,`Tipo: ${typeLabel(item.order_type)}`,`Prioridad: ${prioridad}`,`Campo: ${item.fields?.name??"—"}`,`Superficie total: ${item.planned_area?`${number(item.planned_area).toLocaleString("es-AR",{maximumFractionDigits:0})} ha`:"—"}`,`Fecha prevista: ${item.scheduled_date?formatDate(item.scheduled_date):"—"}`,`Responsable: ${responsable}`,`Contratista: ${item.contractors?.name??"—"}`];const lots=orderLots(item);if(lots.length){lines.push("","LOTES INCLUIDOS");lots.forEach(lot=>lines.push(`${lot.plots?.name??plots.find(p=>p.id===lot.plot_id)?.name??"Lote"}: ${number(lot.planned_area).toLocaleString("es-AR",{maximumFractionDigits:0})} ha`))}const products=item.work_order_products??[];if(products.length){lines.push("","INSUMOS Y DOSIS");products.forEach(product=>{const total=plannedTotal(product.dose,item.planned_area,product.dose_unit);lines.push(`${product.product_name} · ${doseText(product)} · Total: ${total!==null?`${total.toLocaleString("es-AR",{maximumFractionDigits:0})} ${totalUnit(product.dose_unit)}`:product.dose_unit==="cc/100kg"?"Según kg tratados":"—"}`)})}if(item.instructions)lines.push("","INDICACIONES",item.instructions);if(item.notes)lines.push("","NOTAS",item.notes);lines.push("","Documento generado con Growr360 · growr360.com");return lines}
 async function exportSelectedZip(){if(!canExport||!selectedOrders.length){setMessage("No tenés permiso para exportar reportes.");return;}setBulkBusy(true);setMessage("");try{const zip=new JSZip();const rows=[["ID","Título","Estado","Tipo","Campo","Superficie ha","Fecha prevista"],...selectedOrders.map(item=>[item.id,item.title,statusLabel(item.status),typeLabel(item.order_type),item.fields?.name??"",item.planned_area??"",item.scheduled_date?String(item.scheduled_date).slice(0,10):""])];zip.file("resumen.csv",rows.map(row=>row.map(csvCell).join(",")).join("\n"));const folder=zip.folder("ordenes");selectedOrders.forEach((item,index)=>folder?.file(`${String(index+1).padStart(2,"0")}-${safeFileName(item.title)}.pdf`,buildSimplePdf(item.title,orderPdfLines(item))));const blob=await zip.generateAsync({type:"blob",compression:"DEFLATE"});const url=URL.createObjectURL(blob);const anchor=document.createElement("a");anchor.href=url;anchor.download=`growr360-ots-${new Date().toISOString().slice(0,10)}.zip`;anchor.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch(error){setMessage(spanishError(error))}finally{setBulkBusy(false)}}
 async function deleteSelectedOrders(){if(!deletableSelected.length)return;const skipped=selectedOrders.length-deletableSelected.length;const text=`¿Eliminar ${deletableSelected.length} OT${deletableSelected.length===1?"":"s"} seleccionada${deletableSelected.length===1?"":"s"}?${skipped?` ${skipped} no se eliminará${skipped===1?"":"n"} porque no tenés permiso.`:""} Los registros generados por OTs finalizadas se conservarán.`;if(!window.confirm(text))return;setBulkBusy(true);setMessage("");try{for(const item of deletableSelected){const result=await supabase.rpc("delete_work_order",{p_work_order_id:item.id});if(result.error)throw result.error}setSelectedIds(current=>current.filter(id=>!deletableSelected.some(item=>item.id===id)));await Promise.resolve(onSaved())}catch(error){setMessage(spanishError(error))}finally{setBulkBusy(false)}}
 ...
 const plannedTotal=(dose:unknown,area:unknown,unit?:string|null)=>{const raw=String(unit??"").toLowerCase();if(raw==="cc/100kg")return null;const d=number(dose as string|number),a=number(area as string|number);if(!(d>0&&a>0))return null;const total=d*a;return raw==="cc/ha"?total/1000:raw==="gr/ha"?total/1000:total};
 const totalUnit=(unit?:string|null)=>{const raw=String(unit??"").toLowerCase();if(raw==="cc/ha")return "lt";if(raw==="gr/ha")return "kg";return raw.endsWith("/ha")?raw.replace(/\/ha$/i,""):raw==="cc/100kg"?"":"u"};
 const doseText=(product:WorkOrderProduct)=>product.dose?`${number(product.dose).toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})} ${product.dose_unit??""}`:"—";
 const addDraftProduct=()=>{const nextIndex=draft.products.length;setCollapsedProducts(Object.fromEntries(draft.products.map((_,index)=>[index,true])));setDraft({...draft,products:[...draft.products,{input_id:"",product_name:"",dose:"",dose_unit:"kg/ha",planned_quantity:""}]});requestAnimationFrame(()=>requestAnimationFrame(()=>document.querySelector(`[data-product-index="${nextIndex}"]`)?.scrollIntoView({behavior:"smooth",block:"center"}))) };
 function openCreate(){setSelected(null);setDraft(blank());setCollapsedProducts({});setMessage("");setCollapsedProducts({});setFormStep(0);setMode("form")}
 ...
 async function deleteOrder(item:WorkOrder){if(!mayDelete(item))return;const warning=normalizedStatus(item.status)==="completed"?`¿Eliminar la OT “${item.title}”? Los registros generados al finalizarla se conservarán.`:`¿Eliminar la OT “${item.title}”? Esta acción la quitará de las órdenes de trabajo.`;if(!window.confirm(warning))return;setBusy(true);setMessage("");const result=await supabase.rpc("delete_work_order",{p_work_order_id:item.id});setBusy(false);if(result.error){setMessage(spanishError(result.error));return}setMode(null);setSelected(null);onSaved()}
 ...
 function printWorkOrder(){if(!selected)return;const bytes=buildSimplePdf(selected.title,orderPdfLines(selected));const blob=new Blob([bytes],{type:"application/pdf"});const url=URL.createObjectURL(blob);const anchor=document.createElement("a");anchor.href=url;anchor.download=`${safeFileName(selected.title)}.pdf`;anchor.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
 const typeLabel=(v:string)=>({sowing:"Siembra",spraying:"Pulverización",fertilization:"Fertilización",harvest:"Cosecha",work:"Roturación / labor",soil_analysis:"Análisis de suelo",other:"Otro"}[v]??v);const statusLabel=(v:WorkOrder["status"])=>normalizedStatus(v)==="completed"?"Finalizada":normalizedStatus(v)==="cancelled"?"Cancelada":"Pendiente";
 ...
 return <div className="page-content work-orders-page work-orders-v64"><PageHead title="Órdenes de trabajo" text="Planificá, administrá y exportá órdenes de trabajo por lote."/><div className="work-order-kpis"><article><span>Pendientes</span><strong>{stats.pending}</strong></article><article className={stats.overdue?"alert":""}><span>Vencidas</span><strong>{stats.overdue}</strong></article><article><span>Finalizadas</span><strong>{stats.done}</strong></article><article><span>Canceladas</span><strong>{stats.cancelled}</strong></article></div><section className="content-card work-order-directory work-order-directory-v64"><header className="work-order-toolbar-v64"><div className="work-order-search"><Search/><input placeholder="Buscar por orden, campo o lote" value={query} onChange={e=>setQuery(e.target.value)}/></div><select value={status} onChange={e=>setStatus(e.target.value)}><option value="pending">Pendientes</option><option value="completed">Finalizadas</option><option value="cancelled">Canceladas</option><option value="all">Todas</option></select>{canCreate&&<button className="primary-action" onClick={openCreate}><Plus/>Nueva orden</button>}</header>{selectedIds.length>0&&<div className="work-order-bulkbar-v64"><div><strong>{selectedIds.length} seleccionada{selectedIds.length===1?"":"s"}</strong><button onClick={()=>setSelectedIds([])}>Limpiar</button></div><div>{canExport&&<button disabled={bulkBusy} onClick={()=>void exportSelectedZip()}>{bulkBusy?<LoaderCircle className="spin"/>:<Download/>}Exportar ZIP</button>}{deletableSelected.length>0&&<button className="danger-text" disabled={bulkBusy} onClick={()=>void deleteSelectedOrders()}><Trash2/>Eliminar {deletableSelected.length}</button>}</div></div>}<div className="work-order-table-head-v64"><label className="work-order-check-v64"><input type="checkbox" checked={allFilteredSelected} onChange={toggleAllFiltered} aria-label="Seleccionar órdenes visibles"/><span/></label><span>Orden</span><span>Campo / lotes</span><span>Superficie</span><span>Estado</span><span/></div><div className="work-order-list work-order-list-v64">{filtered.map(item=>{const lots=orderLots(item);const checked=selectedIds.includes(item.id);return <article key={item.id} className={`work-order-row-v64${checked?" is-selected":""}`}><label className="work-order-check-v64" onClick={event=>event.stopPropagation()}><input type="checkbox" checked={checked} onChange={()=>toggleSelected(item.id)} aria-label={`Seleccionar ${item.title}`}/><span/></label><button className="work-order-main-v64" onClick={()=>{setSelected(item);setMode("view");setMessage("")}}><i className={`priority-${item.priority}`}/><div><small>{typeLabel(item.order_type)}{item.scheduled_date?` · ${formatDate(item.scheduled_date)}`:""}</small><strong>{item.title}</strong></div></button><button className="work-order-cell-v64" onClick={()=>{setSelected(item);setMode("view");setMessage("")}}><strong>{item.fields?.name??"Sin campo"}</strong><small>{lots.length?`${lots.length} lote${lots.length===1?"":"s"}`:item.plots?.name??"—"}</small></button><button className="work-order-area-v64" onClick={()=>{setSelected(item);setMode("view");setMessage("")}}>{item.planned_area?`${number(item.planned_area).toLocaleString("es-AR",{maximumFractionDigits:0})} ha`:"—"}</button><button className="work-order-status-v64" onClick={()=>{setSelected(item);setMode("view");setMessage("")}}><em className={`status-${normalizedStatus(item.status)}`}>{statusLabel(item.status)}</em></button><button className="work-order-open-v64" onClick={()=>{setSelected(item);setMode("view");setMessage("")}} aria-label={`Abrir ${item.title}`}><ChevronRight/></button></article>})}{!filtered.length&&<EmptyLine text="No hay órdenes con estos filtros."/>}</div>{message&&<p className="form-error work-order-list-error-v64">{message}</p>}</section>
 {mode&&<div className="record-detail-backdrop"><article className={`record-detail-sheet work-order-sheet${selected?` status-${normalizedStatus(selected.status)}`:""}`}><div className="print-only-brand-header"><img src="/favicon.svg" alt="Growr360"/><span>Growr<span className="print-brand-accent">360</span></span></div><header><div><span className="eyebrow">ORDEN DE TRABAJO</span><h2>{mode==="form"?(selected?"Editar orden":"Nueva orden"):selected?.title}</h2></div><button className="icon-button" onClick={()=>{setMode(null);setSelected(null)}}><X/></button></header>
 {mode==="form"?<form className="work-order-form work-order-wizard" onSubmit={save}>{message&&<p className="form-error work-order-error">{message}</p>}<div className="record-wizard"><WizardHead step={formStep+1} total={5} title={formTitle}/>
  ...
  {formStep===3&&<section className="wizard-card work-order-products-step"><span>PASO 4</span><h3>Insumos planificados</h3><p className="wizard-note">La dosis se guarda con su unidad exacta. El total solo se calcula automáticamente para unidades por hectárea.</p><section className="work-order-products"><header><div><strong>Productos</strong><small>{draft.products.length} insumo{draft.products.length===1?"":"s"} · {totalPlannedArea.toLocaleString("es-AR",{maximumFractionDigits:0})} ha</small></div><button type="button" onClick={addDraftProduct}><Plus/>Agregar insumo</button></header>{draft.products.map((p,index)=>{const total=plannedTotal(p.dose,totalPlannedArea,p.dose_unit);const collapsed=Boolean(collapsedProducts[index]);return <article className={`work-order-product-editor${collapsed?" is-collapsed":""}`} data-product-index={index} key={index}><header><button type="button" className="work-order-product-toggle" onClick={()=>setCollapsedProducts(current=>({...current,[index]:!collapsed}))} aria-expanded={!collapsed}><span><strong>{p.product_name.trim()||`Producto ${index+1}`}</strong><small>{p.dose?`${number(p.dose).toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})} ${p.dose_unit||""}`:"Sin dosis definida"}</small></span><ChevronDown/></button><button type="button" className="work-order-product-remove" aria-label="Quitar producto" onClick={()=>{setDraft({...draft,products:draft.products.filter((_,i)=>i!==index)});setCollapsedProducts({})}}><X/></button></header>{!collapsed&&<div className="work-order-product-fields"><label>Seleccionar insumo<select value={p.input_id} onChange={e=>{const supply=supplies.find(s=>s.id===e.target.value);const next=[...draft.products];next[index]={...p,input_id:e.target.value,product_name:supply?.name??p.product_name,dose_unit:normalizeDoseUnit(supply?.unit)||p.dose_unit||"kg/ha"};setDraft({...draft,products:next})}}><option value="">Producto manual</option>{supplies.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label>Producto<input placeholder="Nombre" value={p.product_name} onChange={e=>{const next=[...draft.products];next[index]={...p,product_name:e.target.value};setDraft({...draft,products:next})}}/></label><div className="form-pair"><label>Dosis<input type="number" min="0" step="0.001" placeholder="0" value={p.dose} onChange={e=>{const next=[...draft.products];next[index]={...p,dose:e.target.value};setDraft({...draft,products:next})}}/></label><label>Unidad<select value={p.dose_unit} onChange={e=>{const next=[...draft.products];next[index]={...p,dose_unit:e.target.value};setDraft({...draft,products:next})}}><option value="">Seleccionar…</option>{doseUnits.map(unit=><option key={unit} value={unit}>{unit}</option>)}</select></label></div><div className="work-order-product-total"><small>TOTAL PLANIFICADO</small><strong>{total!==null?`${total.toLocaleString("es-AR",{maximumFractionDigits:0})} ${totalUnit(p.dose_unit)}`:p.dose_unit==="cc/100kg"?"Depende de los kg tratados":"—"}</strong></div></div>}</article>})}{!draft.products.length&&<div className="work-order-products-empty"><Leaf/><strong>Sin insumos todavía</strong><small>Agregá el primer producto para definir dosis y unidad.</small></div>}</section><div className="work-order-products-actions"><button type="button" className="secondary-action" onClick={()=>setFormStep(2)}>Volver</button><button type="button" className="work-order-add-product-bottom" onClick={addDraftProduct}><Plus/>Agregar insumo</button><button type="button" className="primary-action" onClick={()=>setFormStep(4)}>Continuar<ChevronRight/></button></div></section>}
  {formStep===4&&<section className="wizard-card"><span>PASO 5</span><h3>Revisá la orden</h3><div className="record-review"><strong>{draft.title||"Orden sin título"}</strong><small>{typeLabel(draft.order_type)}{selectedField?` · ${selectedField.name}`:""}</small><small>{draft.plot_ids.length} lote{draft.plot_ids.length===1?"":"s"} · {totalPlannedArea.toLocaleString("es-AR",{maximumFractionDigits:0})} ha</small><small>{selectedPlots.map(plot=>plot.name).join(" · ")}</small>{draft.scheduled_date&&<small>{formatDate(draft.scheduled_date)}{draft.scheduled_end_date?` → ${formatDate(draft.scheduled_end_date)}`:""}</small>}{draft.products.filter(p=>p.product_name.trim()).length>0&&<small>{draft.products.filter(p=>p.product_name.trim()).length} producto{draft.products.filter(p=>p.product_name.trim()).length===1?"":"s"}</small>}</div><label className="work-order-member-edits wizard-check"><input type="checkbox" checked={Boolean(draft.allow_member_edits)} onChange={e=>setDraft({...draft,allow_member_edits:e.target.checked})}/><span><strong>Permitir edición por el equipo</strong><small>Otros miembros con permiso de edición podrán modificar, iniciar o cancelar esta orden.</small></span></label><button type="button" className="wizard-back-only" onClick={()=>setFormStep(3)}>Volver a editar</button></section>}
...
const permissionCatalog=[
  ...
  ["delete_own_records","Eliminar registros y OTs propios"],["delete_any_records","Eliminar cualquier registro u OT"],
...
function roleDefault(role:string,permission:string){if(role==="owner")return true;if(role==="admin")return true;if(role==="agronomist")return ["view_fields","view_records","create_records","edit_records","delete_own_records","create_monitoring","view_satellite","view_ndvi","export_reports"].includes(permission);if(role==="operator")return ["view_fields","view_records","create_records","create_monitoring"].includes(permission);return ["view_fields","view_records"].includes(permission)}
...
function RealTeamView({ section, groupId, members, fields, plots, currentRole, currentUserId, canManage, memberLimit, onSection, onSaved }: { section:"members"|"requests"|"invitations"; groupId:string; members: Member[]; fields:Field[];plots:Plot[];currentRole:string; currentUserId:string; canManage:boolean; memberLimit:number|null; onSection:(view:View)=>void; onSaved:()=>void }) {
  ...
  async function role(userId:string,nextRole:string){if(userId===currentUserId){setMessage("No podés cambiar tu propio rol.");return;}setBusy(userId);const {error}=await supabase.rpc("change_member_role",{p_group_id:groupId,p_user_id:userId,p_role:nextRole});setBusy("");if(error)setMessage(error.message);else onSaved();}
  async function remove(userId:string){if(userId===currentUserId){setMessage("No podés quitarte ni modificarte desde la administración del equipo.");return;}if(!confirm("¿Quitar este usuario del grupo?"))return;setBusy(userId);const {error}=await supabase.rpc("remove_group_member",{p_group_id:groupId,p_user_id:userId});setBusy("");if(error)setMessage(error.message);else{setSelected(null);onSaved();}}
  function openMember(member:Member){if(member.user_id===currentUserId){setMessage("No podés modificar tu propio rol, permisos ni alcance de trabajo. Esos accesos los administra otro responsable del grupo.");return;}setSelected(member);setDraft(Object.fromEntries(permissionCatalog.map(([key])=>[key,member.member_permission_overrides?.find(item=>item.permission===key)?.allowed??roleDefault(member.role,key)])));setResourceFieldIds(new Set((member.member_resource_access??[]).map(item=>item.field_id).filter((id):id is string=>Boolean(id))));setResourcePlotIds(new Set((member.member_resource_access??[]).map(item=>item.lot_id).filter((id):id is string=>Boolean(id))))}
  async function savePermissions(){if(!selected)return;if(selected.user_id===currentUserId){setMessage("No podés modificar tus propios permisos.");setSelected(null);return;}setBusy(selected.user_id);setMessage("");const permissions=Object.fromEntries(permissionCatalog.map(([permission])=>[permission,Boolean(draft[permission])]));const [permissionResult,resourceResult]=await Promise.all([supabase.rpc("set_member_permission_overrides",{p_group_id:groupId,p_user_id:selected.user_id,p_permissions:permissions}),supabase.rpc("set_member_resource_access",{p_group_id:groupId,p_user_id:selected.user_id,p_field_ids:[...resourceFieldIds],p_lot_ids:[...resourcePlotIds],p_access_level:"write"})]);setBusy("");const error=permissionResult.error??resourceResult.error;if(error)setMessage(error.message);else{setSelected(null);onSaved();}}
...
function translatedDetailText(value:unknown):string{
  const text=String(value??"").trim();
  const translations:Record<string,string>={
    "custom fine weed":"Otra maleza de hoja fina","custom_fine_weed":"Otra maleza de hoja fina","custom broad weed":"Otra maleza de hoja ancha","custom_broad_weed":"Otra maleza de hoja ancha","custom weed":"Otra maleza","custom_weed":"Otra maleza",
    "sowing":"Siembra","spraying":"Pulverización","fertilization":"Fertilización","harvest":"Cosecha","work":"Roturación / labor","monitoring":"Monitoreo","soil analysis":"Análisis de suelo","soil_analysis":"Análisis de suelo","water table":"Napa","water_table":"Napa","other":"Otro",
    "active":"Activa","inactive":"Inactiva","pending":"Pendiente","completed":"Finalizada","cancelled":"Cancelada","draft":"Borrador","in progress":"En curso","in_progress":"En curso",
    "low":"Baja","normal":"Normal","medium":"Media","high":"Alta","urgent":"Urgente","critical":"Crítica",
    "yes":"Sí","no":"No","true":"Sí","false":"No","inside":"Dentro del lote","outside":"Fuera del lote","inside plot":"Dentro del lote","outside plot":"Fuera del lote",
    "latest plot activity":"Última actividad del lote","latest_plot_activity":"Última actividad del lote","manual":"Manual","automatic":"Automático","broadcast":"Al voleo","banded":"En bandas","foliar":"Foliar","soil":"Al suelo","seed":"Semilla"
  };
  const translatePart=(part:string)=>{const clean=part.trim();const normalized=normalizeText(clean);if(translations[normalized])return translations[normalized];if(/^[a-z0-9_ -]+$/i.test(clean)&&(/[ _]/.test(clean))){const words=clean.replace(/_/g," ").split(/\s+/).map(word=>DETAIL_WORDS[word.toLowerCase()]??word.toLowerCase());return cap(words.join(" "));}return clean};
  if((text.startsWith("{")&&text.endsWith("}"))||(text.startsWith("[")&&text.endsWith("]"))){try{const parsed=JSON.parse(text);if(Array.isArray(parsed))return parsed.map(item=>translatedDetailText(item)).join(", ");if(parsed&&typeof parsed==="object")return Object.entries(parsed as Record<string,unknown>).map(([key,val])=>`${detailLabel(key)}: ${translatedDetailText(val)}`).join(" · ")}catch{}}
  return text.split(/\s*[,|]\s*/).map(translatePart).join(", ");
}
