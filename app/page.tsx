...
type RecordRow = {
  ...
  contractor?: string | null; machinery_text?: string | null; created_at?: string | null;
...
function AuthScreen({ client }: { client: SupabaseClient }) {
  ...
  return <div className="auth-page auth-page-v2">
    ...
    <aside className="auth-showcase"><Brand/><span className="public-kicker"><Leaf/> Gestión agrícola inteligente</span><h2>Tu campo, conectado en un solo lugar.</h2><p>Accedé a los mismos lotes, registros, monitoreos e imágenes satelitales desde cualquier dispositivo.</p><div className="auth-mini-map"><div className="auth-mini-lot one">Lote 1</div><div className="auth-mini-lot two">Lote 2</div></div><small><ShieldCheck/> Información protegida por roles y permisos</small></aside>
...
function PublicLanding({ onLogin, onRegister }: { onLogin: () => void; onRegister: () => void }) {
  return <div className="public-site">
    ...
    <main>
      <section className="public-hero"><span className="public-kicker"><Leaf/> Tecnología argentina para el campo</span><h1>Toda tu gestión agrícola,<br/><em>en un solo lugar.</em></h1><p>Campos, registros, monitoreos GPS, mapas, reportes e imágenes Sentinel-2 para tomar decisiones con información clara, desde el celular o la computadora.</p><div className="hero-actions"><button className="public-cta" onClick={onRegister}>Empezar ahora <ArrowRight/></button><a href="#producto">Conocer Growr360 <ChevronDown/></a></div><div className="hero-visual"><div className="hero-map-grid"/><div className="hero-fields"><div className="hero-lot lot-a"><div className="ndvi-texture ndvi-one"/><span>Lote 1 <small>NDVI contrastado · 42,8 ha</small></span></div><div className="hero-lot lot-b"><div className="ndvi-texture ndvi-two"/><span>Lote 2 <small>NDVI contrastado · 31,2 ha</small></span></div></div><div className="ndvi-legend"><strong>NDVI contrastado</strong><i/><small><span>Bajo</span><span>Alto</span></small></div><div className="hero-dashboard"><TrendingUp/><div><strong>Producción conectada</strong><small>Información actualizada para todo el equipo</small></div></div></div></section>
      ...
      <section className="analytics-public"><div className="analytics-copy"><span>REPORTES Y GRÁFICOS</span><h2>Todo lo que registrás se convierte en información útil.</h2><p>Armá reportes a medida y entendé cómo evoluciona tu producción. Elegí qué comparar, cómo agruparlo y el período que querés analizar.</p><ul><li><Check/>Campañas, campos, lotes y cultivos</li><li><Check/>Superficie, costos, rendimiento y profundidad de napa</li><li><Check/>Contratistas, tipos de trabajo y evolución temporal</li></ul><div className="analytics-tags"><span>Barras</span><span>Líneas</span><span>Torta</span><span>Filtros múltiples</span></div></div><div className="analytics-demo"><header><div><small>EVOLUCIÓN PRODUCTIVA</small><strong>Rendimiento por campaña</strong></div><button><SlidersHorizontal/> Filtros</button></header><div className="analytics-kpis"><div><small>Promedio</small><strong>4.280 kg/ha</strong><em>+12,4%</em></div><div><small>Mejor lote</small><strong>Lote 2</strong><em>5.140 kg/ha</em></div><div><small>Superficie</small><strong>186 ha</strong><em>analizadas</em></div></div><div className="line-chart-demo"><div className="chart-y"><span>6.000</span><span>4.000</span><span>2.000</span><span>0</span></div><svg viewBox="0 0 720 250" role="img" aria-label="Ejemplo de evolución del rendimiento por campaña"><defs><linearGradient id="areaFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#35b864" stopOpacity=".38"/><stop offset="1" stopColor="#35b864" stopOpacity="0"/></linearGradient></defs><path className="chart-area" d="M20 205 C90 190,115 160,175 168 S280 115,350 130 S450 88,520 98 S625 38,700 54 L700 230 L20 230 Z"/><path className="chart-line" d="M20 205 C90 190,115 160,175 168 S280 115,350 130 S450 88,520 98 S625 38,700 54"/><g className="chart-points"><circle cx="20" cy="205" r="6"/><circle cx="175" cy="168" r="6"/><circle cx="350" cy="130" r="6"/><circle cx="520" cy="98" r="6"/><circle cx="700" cy="54" r="7"/></g></svg><div className="chart-x"><span>22/23</span><span>23/24</span><span>24/25</span><span>25/26</span><span>26/27</span></div></div><footer><span><i/>Rendimiento</span><strong>Datos ilustrativos</strong></footer></div></section>
...
function AuthenticatedApp({ session }: { session: Session }) {
  ...
  const loadGroupData = useCallback(async (targetGroup: string, quiet = false) => {
    ...
    const [fieldResult, plotResult, recordResult, memberResult, cropResult, assignmentResult, colorResult, settingsResult, campaignResult, clientResult, supplyResult,contractorResult] = await Promise.all([
      ...
      supabase.from("records").select("id,record_type,record_date,worked_area,contractor,machinery_text,created_at,field_id,plot_id,campaign_id,fields(name),plots(name),campaigns(id,name),sowing_records(data),spraying_records(data),fertilization_records(data),harvest_records(data),work_records(data),monitoring_records(data),expense_records(data),other_records(data)").eq("group_id", targetGroup).is("deleted_at", null).order("record_date", { ascending: false }).limit(500),
      supabase.from("group_members").select("user_id,role,status,profiles!group_members_user_id_fkey(id,first_name,last_name,username,email),member_permission_overrides(permission,allowed)").eq("group_id", targetGroup).eq("status", "active").order("created_at"),
...
function RealMapView({ fields, plots, records, campaigns, assignments, cropColors, crops, selectedPlot, setSelectedPlot, groupId, userId, canManageLots, onCreateRecord, onSaved }: {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      map.addLayer({ id: "sentinel-layer", type: "raster", source: "sentinel-image", paint: { "raster-opacity": .82, "raster-fade-duration": 180 } });
...
function RecordDetail({ record, onClose }: { record: RecordRow; onClose: () => void }) {
  ...
  const visible=Object.entries(details).filter(([key,value])=>key!=="record_kind"&&value!==null&&value!=="");
  return <div className="record-detail-backdrop"><article className="record-detail-sheet"><header><button className="icon-button" onClick={onClose}><ChevronLeft/></button><div><span className="eyebrow">{actualType === "monitoring" ? "MONITOREO" : actualType === "napa" ? "MEDICIÓN DE NAPA" : "REGISTRO"}</span><h2>{recordType(actualType)}{recordCrop(record)&&actualType!=="napa" ? ` · ${recordCrop(record)}` : ""}</h2><p>{relation(record.fields)?.name || "Campo"} · {relation(record.plots)?.name || "Sin lote"}</p></div><button className="icon-button" onClick={onClose}><X/></button></header><div className="detail-hero"><CalendarDays/><div><small>Fecha</small><strong>{formatDate(record.record_date)}</strong></div><div><small>Campaña</small><strong>{relation(record.campaigns)?.name || "Sin campaña"}</strong></div>{actualType!=="napa"&&<div><small>Superficie</small><strong>{number(record.worked_area).toLocaleString("es-AR")} ha</strong></div>}</div><section><h3>Información registrada</h3><div className="detail-grid">{visible.map(([key,value]) => <div key={key}><small>{detailLabel(key)}</small><strong>{formatDetailValue(key,value)}</strong></div>)}{!visible.length && <EmptyLine text="Este registro no tiene datos adicionales."/>}</div></section>{(record.contractor||record.machinery_text)&&<section><h3>Ejecución</h3><div className="detail-grid">{record.contractor&&<div><small>Contratista</small><strong>{record.contractor}</strong></div>}{record.machinery_text&&<div><small>Maquinaria</small><strong>{record.machinery_text}</strong></div>}</div></section>}</article></div>;
}
  ...
  return <div className="page-content"><PageHead title="Campos" text="Abrí un campo para consultar sus lotes, superficie y cultivos." action={canCreate?<button className="primary-action" onClick={onCreate}><Plus/>Crear campo</button>:undefined}/>
    ...
    <div className="field-stack">{fields.map(field => {
      ...
      return <section className={`field-card ${expanded ? "expanded" : ""}`} key={field.id}>
        ...
        {expanded && <div className="field-plots"><div className="field-full-info"><div><small>Superficie total</small><strong>{number(field.total_area).toLocaleString("es-AR",{maximumFractionDigits:2})} ha</strong></div><div><small>Superficie sembrable</small><strong>{number(field.arable_area).toLocaleString("es-AR",{maximumFractionDigits:2})} ha</strong></div><div><small>Ubicación</small><strong>{[field.locality,field.province].filter(Boolean).join(", ")||"Sin datos"}</strong></div></div>{children.map(plot => <button className="field-plot-row" key={plot.id} onClick={() => onOpenPlot(plot)}>
...
function NapaView({records,canCreate,onCreate}:{records:RecordRow[];canCreate:boolean;onCreate:()=>void}){
  const [selected,setSelected]=useState<RecordRow|null>(null);const rows=records.filter(row=>effectiveRecordType(row)==="napa").sort((a,b)=>b.record_date.localeCompare(a.record_date));
  return <div className="page-content"><PageHead title="Napas" text="Historial de profundidad de napa, separado de las labores." action={canCreate?<button className="primary-action" onClick={onCreate}><Plus/>Nueva medición</button>:undefined}/><div className="napa-list">{rows.map(row=><button className="napa-card" key={row.id} onClick={()=>setSelected(row)}><div className="napa-icon"><Waves/></div><div><small>{relation(row.fields)?.name||"Campo"} · {relation(row.plots)?.name||"Sin lote"}</small><h3>{formatNapaDepth(recordData(row).water_table_depth)}</h3><span>{relation(row.campaigns)?.name||"Sin campaña"}</span></div><time>{formatDate(row.record_date)}</time><ChevronRight/></button>)}{!rows.length&&<EmptyLine text="Todavía no hay mediciones de napa."/>}</div>{selected&&<RecordDetail record={selected} onClose={()=>setSelected(null)}/>}</div>;
}
...
function CampaignDetail({campaign,records,onClose}:{campaign:Campaign;records:RecordRow[];onClose:()=>void}){const types=Array.from(new Set(records.map(effectiveRecordType)));return <div className="record-detail-backdrop"><article className="record-detail-sheet compact-detail"><header><div><span className="eyebrow">CAMPAÑA</span><h2>{campaign.name}</h2><p>{campaign.status==="active"?"Activa":campaign.status==="planned"?"Planificada":"Finalizada"}</p></div><button className="icon-button" onClick={onClose}><X/></button></header><div className="detail-hero"><CalendarDays/><div><small>Inicio</small><strong>{formatDate(campaign.start_date)}</strong></div><div><small>Fin</small><strong>{formatDate(campaign.end_date)}</strong></div><div><small>Registros</small><strong>{records.length}</strong></div></div><section><h3>Actividad de la campaña</h3><div className="detail-grid">{types.map(type=><div key={type}><small>{recordType(type)}</small><strong>{records.filter(row=>effectiveRecordType(row)===type).length}</strong></div>)}{!types.length&&<EmptyLine text="Esta campaña todavía no tiene actividad."/>}</div></section></article></div>}
...
function RealReportsView({ fields, plots, records, crops }: { fields: Field[]; plots: Plot[]; records: RecordRow[]; crops: Crop[] }) {
  ...
  const filtered = records.filter(row => effectiveRecordType(row)!=="monitoring" && (!fieldId || row.field_id === fieldId) && (!plotId || row.plot_id === plotId) && (!campaignId || row.campaign_id === campaignId) && (!type || effectiveRecordType(row) === type) && (!contractor || normalizeText(row.contractor ?? "") === normalizeText(contractor)) && (!crop || recordCrop(row).toLowerCase() === crop.toLowerCase()));
  ...
  return <div className="page-content"><PageHead title="Reportes" text="El mismo análisis operativo de la app, con filtros combinables."/>
    <div className="report-filter premium-filter"><select value={fieldId} onChange={e => { setFieldId(e.target.value); setPlotId(""); }}><option value="">Todos los campos</option>{fields.map(field => <option key={field.id} value={field.id}>{field.name}</option>)}</select><select value={plotId} onChange={e => setPlotId(e.target.value)}><option value="">Todos los lotes</option>{plots.filter(plot => !fieldId || plot.field_id === fieldId).map(plot => <option key={plot.id} value={plot.id}>{plot.name}</option>)}</select><select value={campaignId} onChange={e => setCampaignId(e.target.value)}><option value="">Todas las campañas</option>{Array.from(new globalThis.Map(records.map(row=>[row.campaign_id,relation(row.campaigns)?.name])).entries()).filter(([id])=>id).map(([id,name])=><option key={id!} value={id!}>{name}</option>)}</select><select value={crop} onChange={e => setCrop(e.target.value)}><option value="">Todos los cultivos</option>{crops.map(item => <option key={item.id} value={item.name}>{item.name}</option>)}</select><select value={type} onChange={e => setType(e.target.value)}><option value="">Todos los registros</option>{Array.from(new Set(records.map(effectiveRecordType))).map(item => <option key={item} value={item}>{recordType(item)}</option>)}</select><select value={contractor} onChange={e => setContractor(e.target.value)}><option value="">Todos los contratistas</option>{contractors.map(item=><option key={item} value={item}>{item}</option>)}</select><button onClick={() => { setFieldId(""); setPlotId(""); setCampaignId(""); setCrop(""); setType(""); setContractor(""); }}><RotateCcw/>Limpiar</button></div>
...
function ReportChart({type,rows,metric,selected,onSelect}:{type:"bar"|"line"|"pie";rows:[string,number][];metric:string;selected:string;onSelect:(label:string)=>void}){
  ...
  if(type==="line"){const width=760,height=260,pad=34;const points=rows.map(([,value],index)=>`${pad+(rows.length===1?.5:index/(rows.length-1))*(width-pad*2)},${height-pad-(value/max)*(height-pad*2)}`).join(" ");return <div className="line-chart-wrap"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Gráfico de líneas"><line x1={pad} y1={height-pad} x2={width-pad} y2={height-pad}/><line x1={pad} y1={pad} x2={pad} y2={height-pad}/><polyline points={points}/>{points.split(" ").map((point,index)=>{const[x,y]=point.split(",");return <circle className={selected===rows[index][0]?"selected":""} onClick={()=>onSelect(rows[index][0])} key={rows[index][0]} cx={x} cy={y} r="6"><title>{rows[index][0]}: {format(rows[index][1])}</title></circle>})}</svg><div className="line-labels">{rows.map(([label,value])=><button className={selected===label?"selected":""} onClick={()=>onSelect(label)} key={label}><b>{label}</b><small>{format(value)}</small></button>)}</div></div>}
  return <div className="analytics-bars">{rows.map(([label,value],index)=><button className={selected===label?"selected":""} onClick={()=>onSelect(label)} key={label}><strong>{label}</strong><i><b style={{width:`${Math.abs(value)/max*100}%`,background:CHART_COLORS[index%CHART_COLORS.length]}}/></i><span>{format(value)}</span></button>)}</div>;
}

function ChartDrilldown({label,rows,metric,onClose}:{label:string;rows:RecordRow[];metric:string;onClose:()=>void}){const suffix=metricSuffix(metric);const [selectedId,setSelectedId]=useState("");return <section className="chart-drilldown"><header><div><span className="eyebrow">ORIGEN DEL DATO</span><h4>{label}</h4><p>{rows.length} registro{rows.length===1?"":"s"} · Tocá uno para desplegar sus detalles</p></div><button className="icon-button" onClick={onClose}><X/></button></header><div>{rows.sort((a,b)=>b.record_date.localeCompare(a.record_date)).map(row=><div className={`drilldown-entry ${selectedId===row.id?"expanded":""}`} key={row.id}><button className="drilldown-record" onClick={()=>setSelectedId(current=>current===row.id?"":row.id)} aria-expanded={selectedId===row.id}><div><strong>{recordType(effectiveRecordType(row))}</strong><small>{relation(row.fields)?.name||"Campo"} · {relation(row.plots)?.name||"Sin lote"}</small></div><time>{formatDate(row.record_date)}</time><b>{chartValue(row,metric).toLocaleString("es-AR",{maximumFractionDigits:2})}{suffix}</b><ChevronDown/></button>{selectedId===row.id&&<InlineRecordDetail record={row}/>}</div>)}</div></section>}

function InlineRecordDetail({record}:{record:RecordRow}){const data=recordData(record);const entries=Object.entries(data).filter(([key,value])=>key!=="record_kind"&&value!==null&&value!=="");return <div className="inline-record-detail"><div className="inline-detail-summary"><div><small>Campaña</small><strong>{relation(record.campaigns)?.name||"Sin campaña"}</strong></div><div><small>Fecha</small><strong>{formatDate(record.record_date)}</strong></div><div><small>Superficie</small><strong>{number(record.worked_area).toLocaleString("es-AR",{maximumFractionDigits:2})} ha</strong></div>{record.contractor&&<div><small>Contratista</small><strong>{record.contractor}</strong></div>}{record.machinery_text&&<div><small>Maquinaria</small><strong>{record.machinery_text}</strong></div>}</div>{entries.length>0&&<div className="inline-detail-parameters">{entries.map(([key,value])=><div key={key}><small>{detailLabel(key)}</small><strong>{formatDetailValue(key,value)}</strong></div>)}</div>}</div>}
...
function ManagementView({ groupId, userId, fields, plots, campaigns, clients, contractors, crops, supplies, initialForm, initialRecord, onInitialRecordConsumed, onClose, onSaved }: { groupId: string; userId: string; fields: Field[]; plots: Plot[]; campaigns: Campaign[]; clients: ClientRow[]; contractors:Contractor[]; crops: Crop[]; supplies: Supply[]; canFields: boolean; canLots: boolean; canCampaigns: boolean; canRecords: boolean; initialForm:"field"|"campaign"|"client"|"contractor"|"record"; initialRecord: {plotId:string;type:string}|null; onInitialRecordConsumed:()=>void; onClose:()=>void; onMap: () => void; onSaved: () => void }) {
  ...
  return <div className="creation-host">
    {form&&<div className="record-detail-backdrop"><form className="entity-form" onSubmit={save}><header><div><span className="eyebrow">NUEVA ALTA</span><h2>{formTitle(form)}</h2></div><button type="button" className="icon-button" onClick={onClose}><X/></button></header>
...
function RecordWizard({data,setData,fields,plots,campaigns,contractors,crops,supplies}:{data:Record<string,string>;setData:(value:Record<string,string>)=>void;fields:Field[];plots:Plot[];campaigns:Campaign[];contractors:Contractor[];crops:Crop[];supplies:Supply[]}){
  const [step,setStep]=useState(0);const type=data.record_type||"sowing";const special=["monitoring","napa"].includes(type);
  useEffect(()=>{setData({...data,_wizard_ready:special||step===3?"true":"false"});},[step,type]);
  useEffect(()=>{const input=Array.from(document.querySelectorAll<HTMLInputElement>(".record-wizard input")).find(item=>item.parentElement?.textContent?.startsWith("Contratista"));if(!input)return;input.setAttribute("list","growr-contractors");let list=document.querySelector<HTMLDataListElement>("#growr-contractors");if(!list){list=document.createElement("datalist");list.id="growr-contractors";document.body.appendChild(list)}list.replaceChildren(...contractors.map(contractor=>{const option=document.createElement("option");option.value=contractor.name;option.label=[contractor.document,contractor.phone].filter(Boolean).join(" · ");return option}));const normalize=()=>{const match=contractors.find(item=>normalizeText(item.name)===normalizeText(input.value));if(match&&input.value!==match.name){input.value=match.name;input.dispatchEvent(new Event("input",{bubbles:true}))}};input.addEventListener("change",normalize);return()=>input.removeEventListener("change",normalize)},[step,contractors]);
  useEffect(()=>{const input=Array.from(document.querySelectorAll<HTMLInputElement>(".record-wizard input")).find(item=>item.parentElement?.textContent?.startsWith("Contratista"));if(!input)return;const validate=()=>{const value=input.value.trim();const valid=!value||contractors.some(item=>normalizeText(item.name)===normalizeText(value));input.setCustomValidity(valid?"":"Elegí un contratista cargado en Más → Contratistas.")};input.addEventListener("input",validate);validate();return()=>input.removeEventListener("input",validate)},[step,contractors]);
...
const DETAIL_LABELS:Record<string,string>={crop:"Cultivo",cultivo:"Cultivo",title:"Tipo de registro",observations:"Observaciones",water_table_depth:"Profundidad de napa",phenological_state:"Estado fenológico",monitoring_priority:"Importancia",weeds:"Malezas",weed_levels:"Nivel de malezas",insects:"Insectos",insect_levels:"Nivel de insectos",diseases:"Enfermedades",disease_levels:"Nivel de enfermedades",seed_density:"Densidad de siembra",harvested_area:"Superficie cosechada",total_production:"Producción total",yield_per_ha:"Rendimiento por hectárea",labor_cost_per_ha:"Costo de labor por hectárea",labor_cost:"Costo de labor",application_cost_per_ha:"Costo de aplicación por hectárea",application_cost:"Costo de aplicación",total_cost:"Costo total",inputs_total:"Costo de insumos",input_count:"Cantidad de insumos",soil_sample_count:"Cantidad de muestras",depth_from:"Profundidad desde",depth_to:"Profundidad hasta",organic_matter:"Materia orgánica",ph:"pH",phosphorus:"Fósforo",sulfur:"Azufre",zinc:"Zinc",nitrate:"Nitratos",potassium:"Potasio",cec:"CIC",conductivity:"Conductividad eléctrica",gps_latitude:"Latitud GPS",gps_longitude:"Longitud GPS",gps_accuracy_m:"Precisión GPS"};
Object.assign(DETAIL_LABELS,{sample_depth:"Profundidad de muestreo",sampling_method:"Método de muestreo",laboratory:"Laboratorio",nitrogen:"Nitrógeno",no3:"NO3",ec:"Conductividad eléctrica",recommendations:"Recomendaciones",name:"Nombre",dose:"Dosis por hectárea",price:"Precio unitario",unit:"Unidad",id:"Identificador"});
function detailLabel(key:string){const soil=key.match(/^soil_sample_(\d+)_(.+)$/);if(soil)return `Muestra ${Number(soil[1])+1} · ${DETAIL_LABELS[soil[2]]||cap(soil[2].replaceAll("_"," "))}`;const input=key.match(/^input_(\d+)_(.+)$/);if(input)return `Insumo ${Number(input[1])+1} · ${DETAIL_LABELS[input[2]]||cap(input[2].replaceAll("_"," "))}`;return DETAIL_LABELS[key]||cap(key.replaceAll("_"," "));}
function formatDetailValue(key:string,value:unknown){if(typeof value==="boolean")return value?"Sí":"No";if(key.includes("depth")&&String(value).trim()!=="")return `${number(value as string|number).toLocaleString("es-AR",{maximumFractionDigits:2})} cm`;return String(value);}
