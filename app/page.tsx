...
function buildSimplePdf(title:string,rawLines:string[]){
...
function RealRecordsView({ records, canCreate, onCreate, mode = "records", groupId, userId, canDeleteAny, canDeleteOwn, onSaved }: { records: RecordRow[]; canCreate:boolean; onCreate:()=>void; mode?:"records"|"monitoring"; groupId:string; userId:string; canDeleteAny:boolean; canDeleteOwn:boolean; onSaved:()=>void }) {
  ...
  async function exportSelectedRecords(){if(!selectedRows.length)return;setBulkBusy(true);setBulkMessage("");try{const zip=new JSZip();const folder=zip.folder(monitoring?"monitoreos":"registros");selectedRows.forEach((row,index)=>{const title=`${recordType(effectiveRecordType(row))} · ${relation(row.fields)?.name??"Campo"} · ${relation(row.plots)?.name??"Lote"}`;folder?.file(`${String(index+1).padStart(2,"0")}-${safeExportName(title,"registro")}.pdf`,buildSimplePdf(title,recordPdfLines(row)))});const blob=await zip.generateAsync({type:"blob",compression:"DEFLATE"});const url=URL.createObjectURL(blob);const anchor=document.createElement("a");anchor.href=url;anchor.download=`growr360-${monitoring?"monitoreos":"registros"}-${new Date().toISOString().slice(0,10)}.zip`;anchor.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch(error){setBulkMessage(spanishError(error))}finally{setBulkBusy(false)}}
