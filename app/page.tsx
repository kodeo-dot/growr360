...
function WorkOrdersView({groupId,userId,orders,records,fields,plots,campaigns,members,contractors,supplies,plan,canCreate,canEditAny,canEditOwn,canDeleteAny,canDeleteOwn,onSaved}:{groupId:string;userId:string;orders:WorkOrder[];records:RecordRow[];fields:Field[];plots:Plot[];campaigns:Campaign[];members:Member[];contractors:Contractor[];supplies:Supply[];plan:string;canCreate:boolean;canEditAny:boolean;canEditOwn:boolean;canDeleteAny:boolean;canDeleteOwn:boolean;onSaved:()=>void}){
 ...
 function orderExportHtml(item:WorkOrder){const lots=orderLots(item);const products=item.work_order_products??[];const facts=[["Estado",statusLabel(item.status)],["Tipo",typeLabel(item.order_type)],["Campo",item.fields?.name??"—"],["Superficie",item.planned_area?`${number(item.planned_area).toLocaleString("es-AR",{maximumFractionDigits:0})} ha`:"—"],["Fecha prevista",item.scheduled_date?formatDate(item.scheduled_date):"—"]];return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(item.title)}</title><style>body{font-family:Arial,sans-serif;color:#18231b;margin:44px;line-height:1.35}header{border-bottom:2px solid #183d2b;padding-bottom:18px;margin-bottom:26px}small{color:#66756b;text-transform:uppercase;letter-spacing:.08em;font-size:10px}h1{font-size:28px;margin:6px 0 0}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:20px 0}.fact{border:1px solid #dfe6e1;padding:12px}.fact b{display:block;margin-top:4px}h2{font-size:15px;margin:26px 0 10px}table{width:100%;border-collapse:collapse}th,td{text-align:left;border-bottom:1px solid #e4e9e5;padding:10px 6px;font-size:13px}th{font-size:10px;color:#66756b}p{font-size:13px}.footer{margin-top:34px;padding-top:12px;border-top:1px solid #dfe6e1;color:#748078;font-size:10px}@media print{body{margin:20mm}}</style></head><body><header><small>Growr360 · Orden de trabajo</small><h1>${escapeHtml(item.title)}</h1></header><div class="grid">${facts.map(([label,value])=>`<div class="fact"><small>${escapeHtml(label)}</small><b>${escapeHtml(value)}</b></div>`).join("")}</div>${lots.length?`<h2>Lotes incluidos</h2><table><thead><tr><th>LOTE</th><th>SUPERFICIE</th></tr></thead><tbody>${lots.map(lot=>`<tr><td>${escapeHtml(lot.plots?.name??plots.find(p=>p.id===lot.plot_id)?.name??"Lote")}</td><td>${escapeHtml(number(lot.planned_area).toLocaleString("es-AR",{maximumFractionDigits:0}))} ha</td></tr>`).join("")}</tbody></table>`:""}${products.length?`<h2>Insumos y dosis</h2><table><thead><tr><th>INSUMO</th><th>DOSIS</th><th>TOTAL OT</th></tr></thead><tbody>${products.map(product=>{const total=plannedTotal(product.dose,item.planned_area,product.dose_unit);return `<tr><td>${escapeHtml(product.product_name)}</td><td>${escapeHtml(doseText(product))}</td><td>${total!==null?`${escapeHtml(total.toLocaleString("es-AR",{maximumFractionDigits:0}))} ${escapeHtml(totalUnit(product.dose_unit))}`:product.dose_unit==="cc/100kg"?"Según kg tratados":"—"}</td></tr>`}).join("")}</tbody></table>`:""}${item.instructions?`<h2>Indicaciones</h2><p>${escapeHtml(item.instructions)}</p>`:""}${item.notes?`<h2>Notas</h2><p>${escapeHtml(item.notes)}</p>`:""}<div class="footer">Orden generada con Growr360 · growr360.com</div></body></html>`}
 async function exportSelectedZip(){if(!selectedOrders.length)return;setBulkBusy(true);setMessage("");try{const zip=new JSZip();const rows=[["ID","Título","Estado","Tipo","Campo","Superficie ha","Fecha prevista"],...selectedOrders.map(item=>[item.id,item.title,statusLabel(item.status),typeLabel(item.order_type),item.fields?.name??"",item.planned_area??"",item.scheduled_date?String(item.scheduled_date).slice(0,10):""])];zip.file("resumen.csv",rows.map(row=>row.map(csvCell).join(",")).join("\n"));const folder=zip.folder("ordenes");selectedOrders.forEach((item,index)=>folder?.file(`${String(index+1).padStart(2,"0")}-${safeFileName(item.title)}.html`,orderExportHtml(item)));zip.file("LEEME.txt","Exportación Growr360. Cada OT está incluida como documento HTML listo para abrir o imprimir/guardar como PDF. resumen.csv contiene el índice general.");const blob=await zip.generateAsync({type:"blob",compression:"DEFLATE"});const url=URL.createObjectURL(blob);const anchor=document.createElement("a");anchor.href=url;anchor.download=`growr360-ots-${new Date().toISOString().slice(0,10)}.zip`;anchor.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch(error){setMessage(spanishError(error))}finally{setBulkBusy(false)}}
