...
function RealMapView({ fields, plots, records, campaigns, assignments, cropColors, crops, selectedPlot, setSelectedPlot, groupId, userId, canManageLots, onCreateRecord, onSaved }: {
  ...
  return <div className="map-workspace">
    ...
    {!drawing && !draft && <div className="map-toolbar">
      ...
      <button onClick={() => satelliteOpen ? setSatelliteOpen(false) : void openSatellite()} className={satelliteOpen ? "selected" : ""} title="Imágenes Sentinel-2 de Copernicus"><Satellite/><span>Copernicus</span></button>
...
function PlansView({plans,subscription,plots,campaigns,assignments,members,groupName}:{plans:SubscriptionPlan[];subscription:GroupSubscription|null;plots:Plot[];campaigns:Campaign[];assignments:PlotCampaign[];members:Member[];groupName:string}){
  ...
  const featureNames:Record<string,string>={campaigns:"Campañas",records:"Registros y monitoreos",tasks:"Tareas",map:"Mapa productivo",plot_drawing:"Trazado e importación de lotes",ndvi_current:"Última imagen NDVI",satellite_latest_ndvi:"Última imagen NDVI",ndvi_history:"Historial NDVI",ndvi_compare:"Comparación NDVI",satellite_history:"Historial satelital",satellite_comparison:"Comparación satelital",satellite_alerts:"Alertas satelitales",advanced_analytics:"Analítica avanzada",pdf_export:"Exportación PDF",excel_export:"Exportación Excel",exports:"Exportaciones",advanced_roles:"Roles avanzados",multi_organization:"Multi-organización",business_reports:"Reportes empresariales",enterprise:"Operación empresarial"};
  ...
  return <div className="page-content plans-page"><PageHead title="Planes Growr" text={`Funciones y límites de la organización ${groupName}.`}/>
    ...
    <div className="plans-grid">{ordered.map(plan=><article key={plan.code} className={`${plan.code===currentCode?"current":""} ${plan.code}`}><header><div><span>{plan.code==="free"?<Leaf/>:plan.code==="pro"?<TrendingUp/>:<BriefcaseBusiness/>}</span><div><small>{plan.code==="free"?"INICIAL":plan.code==="pro"?"PROFESIONAL":"EMPRESA"}</small><h3>{plan.name}</h3><p>{planText(plan)}</p></div></div>{plan.code===currentCode&&<b>Tu plan</b>}</header><div className="plan-price"><strong>{plan.monthly_price_usd?`USD ${plan.monthly_price_usd.toLocaleString("es-AR")}`:"USD 0"}</strong><span>{plan.monthly_price_usd?"por mes":"para siempre"}</span>{plan.annual_price_usd>0&&<small>USD {plan.annual_price_usd.toLocaleString("es-AR")} por año</small>}</div><div className="plan-limits"><span>{`Hasta ${(plan.included_hectares??plan.max_hectares??0).toLocaleString("es-AR")} ha activas incluidas`}</span><span>Campos, lotes, campañas e historial ilimitados</span><span>{plan.max_users==null?"Integrantes ilimitados":`Hasta ${plan.max_users} integrantes`}</span><span>Importaciones KML/KMZ ilimitadas</span>{plan.extra_hectare_price_year_usd>0&&<span>USD {plan.extra_hectare_price_year_usd.toLocaleString("es-AR")} por ha adicional/año</span>}</div><ul>{plan.features.map(feature=><li key={feature}><Check/>{featureNames[feature]??feature}</li>)}</ul><button disabled>{plan.code===currentCode?"Plan actual":"Próximamente"}</button></article>)}</div>
