...
type View = "mapa" | "campos" | "registros" | "campanas" | "reportes" | "equipo" | "configuracion";
...
function AuthenticatedApp({ session }: { session: Session }) {
  ...
  const [pendingForm, setPendingForm] = useState<"field"|"campaign"|"client"|"record"|null>(null);
  ...
  const loadGroupData = useCallback(async (targetGroup: string, quiet = false) => {
    ...
    setError("");
    const [fieldResult, plotResult, recordResult, memberResult, cropResult, assignmentResult, colorResult, settingsResult, campaignResult, clientResult, supplyResult] = await Promise.all([
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {!groupId ? <EmptyWorkspace/> : <>
        ...
        {view === "campanas" && <CampaignsView campaigns={campaigns} canCreate={hasPermission("manage_campaigns")} onCreate={() => setPendingForm("campaign")}/>} 
        ...
        {pendingForm && <ManagementView groupId={groupId} userId={session.user.id} fields={fields} plots={plots} campaigns={campaigns} clients={clients} crops={crops} supplies={supplies} canFields={hasPermission("manage_fields")} canLots={hasPermission("manage_lots")} canCampaigns={hasPermission("manage_campaigns")} canRecords={hasPermission("create_records")} initialForm={pendingForm} initialRecord={pendingRecord} onInitialRecordConsumed={() => setPendingRecord(null)} onClose={() => setPendingForm(null)} onMap={() => { setPendingForm(null); setView("mapa"); }} onSaved={() => { setPendingForm(null); void loadGroupData(groupId, true); }}/>} 
      </>}
    </main>
  </div>;
}
...
function ManagementView({ groupId, userId, fields, plots, campaigns, clients, crops, supplies, initialForm, initialRecord, onInitialRecordConsumed, onClose, onSaved }: { groupId: string; userId: string; fields: Field[]; plots: Plot[]; campaigns: Campaign[]; clients: ClientRow[]; crops: Crop[]; supplies: Supply[]; canFields: boolean; canLots: boolean; canCampaigns: boolean; canRecords: boolean; initialForm:"field"|"campaign"|"client"|"record"; initialRecord: {plotId:string;type:string}|null; onInitialRecordConsumed:()=>void; onClose:()=>void; onMap: () => void; onSaved: () => void }) {
  ...
  return <div className="creation-host">
    {form&&<div className="record-detail-backdrop"><form className="entity-form" onSubmit={save}><header><div><span className="eyebrow">NUEVA ALTA</span><h2>{formTitle(form)}</h2></div><button type="button" className="icon-button" onClick={onClose}><X/></button></header>
      ...
      {form==="record"&&<RecordWizard data={data} setData={setData} fields={fields} plots={plots} campaigns={campaigns} crops={crops} supplies={supplies}/>} 
...
function RecordWizard({data,setData,fields,plots,campaigns,crops,supplies}:{data:Record<string,string>;setData:(value:Record<string,string>)=>void;fields:Field[];plots:Plot[];campaigns:Campaign[];crops:Crop[];supplies:Supply[]}){
  ...
  return <div className="record-wizard"><WizardHead step={step+1} total={4} title={recordType(type)}/>{step===0&&<section className="wizard-card"><span>PASO 1</span><h3>Ubicación del trabajo</h3><label>Tipo de registro<select value={type} onChange={event=>{setData({...data,record_type:event.target.value});setStep(0)}}>{["sowing","spraying","fertilization","harvest","work","napa","soil_analysis","other"].map(value=><option key={value} value={value}>{recordType(value)}</option>)}</select></label>{type==="other"&&<label>¿Qué tipo de registro es?<input required value={data.title||""} onChange={event=>setData({...data,title:event.target.value})}/></label>}{location}<WizardNext enabled={Boolean(data.campaign_id&&data.field_id&&data.plot_id&&data.record_date)} next={()=>setStep(1)}/></section>}{step===1&&<section className="wizard-card"><span>PASO 2</span><h3>Datos de {recordType(type).toLowerCase()}</h3><label>Superficie trabajada (ha)<input required inputMode="decimal" value={data.worked_area||""} onChange={event=>setData({...data,worked_area:event.target.value})}/></label>{type!=="soil_analysis"&&<label>Cultivo<select value={data.crop||""} onChange={event=>setData({...data,crop:event.target.value})}><option value="">Sin cultivo</option>{crops.map(c=><option key={c.id} value={c.name}>{c.name}</option>)}</select></label>}{type==="soil_analysis"?<SoilSamplesEditor data={data} setData={setData}/>:<RecordSpecificFields data={data} setData={setData}/>}<WizardNavigation back={()=>setStep(0)} next={()=>setStep(2)}/></section>}{step===2&&<section className="wizard-card"><span>PASO 3</span><h3>{["sowing","spraying","fertilization"].includes(type)?"Productos y ejecución":"Ejecución del trabajo"}</h3>{["sowing","spraying","fertilization"].includes(type)&&<SupplyLinesEditor supplies={supplies} data={data} setData={setData}/>}<div className="form-pair"><label>Contratista<input value={data.contractor||""} onChange={event=>setData({...data,contractor:event.target.value})}/></label><label>Maquinaria<input value={data.machinery||""} onChange={event=>setData({...data,machinery:event.target.value})}/></label></div><WizardNavigation back={()=>setStep(1)} next={()=>setStep(3)}/></section>}{step===3&&<section className="wizard-card"><span>PASO 4</span><h3>Listo para guardar</h3><div className="record-review"><strong>{recordType(type)}</strong><small>{plots.find(p=>p.id===data.plot_id)?.name} · {data.worked_area} ha</small><small>{campaigns.find(c=>c.id===data.campaign_id)?.name} · {data.record_date}</small></div><label>Observaciones<textarea value={data.observations||""} onChange={event=>setData({...data,observations:event.target.value})}/></label><label className="wizard-check"><input type="checkbox" checked={data.allow_member_edits==="true"} onChange={event=>setData({...data,allow_member_edits:String(event.target.checked)})}/><span>Permitir que otros miembros editen este registro</span></label><button type="button" className="wizard-back-only" onClick={()=>setStep(2)}>Volver a editar</button></section>}</div>;
}
...
function recordReady(data:Record<string,string>){if(data._wizard_ready!=="true")return false;if(!data.campaign_id||!data.field_id||!data.plot_id||!data.record_date)return false;if(data.record_type==="monitoring")return Boolean(data.crop&&data.phenological_state);if(data.record_type==="napa")return number(data.water_table_depth)>0;if(["sowing","spraying","fertilization"].includes(data.record_type)){const lines:InputLine[]=data.input_lines_json?JSON.parse(data.input_lines_json):[];return lines.length>0&&lines.every(line=>line.inputId&&number(line.dose)>0&&number(line.price)>=0)}return true}
...
function subtitle(view: View) { return ({ campos: "Estructura territorial y productiva", registros: "Actividad sincronizada del equipo", campanas: "Ciclos productivos", reportes: "Gráficos del grupo activo", equipo: "Miembros, roles y permisos", configuracion: "Preferencias y grupo", mapa: "" } as Record<View, string>)[view]; }
