...
type WorkOrder = {id:string;group_id:string;campaign_id:string;field_id?:string|null;plot_id?:string|null;order_type:"application"|"sowing"|"harvest"|"activity"|"monitoring"|"other";status:"draft"|"pending"|"in_progress"|"completed"|"cancelled";priority:"low"|"normal"|"high"|"urgent";title:string;instructions?:string|null;notes?:string|null;scheduled_date:string;scheduled_end_date?:string|null;planned_area?:number|null;assigned_to?:string|null;contractor_id?:string|null;planned_data?:Record<string,unknown>;actual_data?:Record<string,unknown>;resulting_record_id?:string|null;created_by:string;completed_at?:string|null;fields?:{name:string}|null;plots?:{name:string}|null;campaigns?:{name:string}|null;profiles?:Profile|null;contractors?:{name:string}|null;work_order_products?:WorkOrderProduct[]};
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
      ...
      <nav className="sidebar-navigation">
        ...
        <section className={`nav-group ${navExpanded === "actividad" ? "expanded" : ""}`}>
          ...
          {navExpanded === "actividad" && <div className="nav-submenu">{sidebarItem("ordenes", "Órdenes de trabajo", BriefcaseBusiness, workOrders.filter(item=>["pending","in_progress"].includes(item.status)).length)}{sidebarItem("registros", "Registros", FileText, records.filter(row => !["monitoring", "napa"].includes(effectiveRecordType(row))).length)}{sidebarItem("monitoreos", "Monitoreos", Eye, records.filter(row => effectiveRecordType(row) === "monitoring").length)}{sidebarItem("campanas", "Campañas", CalendarDays)}{sidebarItem("napas", "Napas", Waves)}</div>}
        </section>
    ...
    <main>
      ...
      {!groupId ? <EmptyWorkspace onGroups={() => setGroupBrowserOpen(true)}/> : <>
        ...
        {view === "planes" && <PlansView plans={plans} subscription={subscription} plots={plots} campaigns={campaigns} assignments={assignments} members={members} groupName={group?.name??"Grupo activo"}/>}
...
function buildChartRows(rows:RecordRow[],dimension:string,metric:string){const grouped=new globalThis.Map<string,{total:number,count:number}>();rows.forEach(row=>{if(metric==="water_table"&&effectiveRecordType(row)!=="napa")return;const label=chartLabel(row,dimension);const current=grouped.get(label)??{total:0,count:0};current.total+=chartValue(row,metric);current.count+=1;grouped.set(label,current);});const result=Array.from(grouped,([label,value])=>[label,metric==="water_table"&&value.count?value.total/value.count:value.total] as [string,number]);return dimension==="month"?result.sort((a,b)=>a[0].localeCompare(b[0])):result.sort((a,b)=>b[1]-a[1]);}
...
function ChartDrilldown({label,rows,metric,onClose}:{label:string;rows:RecordRow[];metric:string;onClose:()=>void}){
  ...
  const total=sum(rows.map(row=>chartValue(row,metric)));
...
function WorkOrdersView({groupId,userId,orders,fields,plots,campaigns,members,contractors,supplies,plan,canCreate,canEditAny,canEditOwn,onSaved}:{groupId:string;userId:string;orders:WorkOrder[];fields:Field[];plots:Plot[];campaigns:Campaign[];members:Member[];contractors:Contractor[];supplies:Supply[];plan:string;canCreate:boolean;canEditAny:boolean;canEditOwn:boolean;onSaved:()=>void}){
 const [status,setStatus]=useState("active");const [query,setQuery]=useState("");const [selected,setSelected]=useState<WorkOrder|null>(null);const [mode,setMode]=useState<"view"|"form"|"complete"|null>(null);const [busy,setBusy]=useState(false);const [message,setMessage]=useState("");
 const blank=()=>({title:"",order_type:"application",status:"pending",priority:"normal",scheduled_date:new Date().toISOString().slice(0,10),scheduled_end_date:"",campaign_id:campaigns.find(c=>c.status==="active")?.id??campaigns[0]?.id??"",field_id:"",plot_id:"",planned_area:"",assigned_to:"",contractor_id:"",instructions:"",notes:"",products:[] as {input_id:string;product_name:string;dose:string;dose_unit:string;planned_quantity:string}[]});
 ...
 const filtered=orders.filter(item=>{const active=["pending","in_progress"].includes(item.status);if(status==="active"&&!active)return false;if(status!=="all"&&status!=="active"&&item.status!==status)return false;return `${item.title} ${item.fields?.name??""} ${item.plots?.name??""}`.toLowerCase().includes(query.toLowerCase())});
 const stats={pending:orders.filter(o=>o.status==="pending").length,inProgress:orders.filter(o=>o.status==="in_progress").length,overdue:orders.filter(o=>["pending","in_progress"].includes(o.status)&&o.scheduled_date<new Date().toISOString().slice(0,10)).length,done:orders.filter(o=>o.status==="completed").length};
 const mayEdit=(item:WorkOrder)=>canEditAny||(canEditOwn&&item.created_by===userId);
 function openCreate(){setSelected(null);setDraft(blank());setMessage("");setMode("form")}
 function openEdit(item:WorkOrder){setSelected(item);setDraft({title:item.title,order_type:item.order_type,status:item.status,priority:item.priority,scheduled_date:item.scheduled_date,scheduled_end_date:item.scheduled_end_date??"",campaign_id:item.campaign_id,field_id:item.field_id??"",plot_id:item.plot_id??"",planned_area:item.planned_area==null?"":String(item.planned_area),assigned_to:item.assigned_to??"",contractor_id:item.contractor_id??"",instructions:item.instructions??"",notes:item.notes??"",products:(item.work_order_products??[]).map(p=>({input_id:p.input_id??"",product_name:p.product_name,dose:p.dose==null?"":String(p.dose),dose_unit:p.dose_unit??"",planned_quantity:p.planned_quantity==null?"":String(p.planned_quantity)}))});setMode("form")}
 async function save(event:FormEvent){event.preventDefault();setBusy(true);setMessage("");const payload={group_id:groupId,campaign_id:draft.campaign_id,field_id:draft.field_id||null,plot_id:draft.plot_id||null,order_type:draft.order_type,status:draft.status,priority:draft.priority,title:draft.title.trim(),instructions:draft.instructions.trim()||null,notes:draft.notes.trim()||null,scheduled_date:draft.scheduled_date,scheduled_end_date:draft.scheduled_end_date||null,planned_area:draft.planned_area?Number(draft.planned_area):null,assigned_to:draft.assigned_to||null,contractor_id:draft.contractor_id||null,created_by:selected?.created_by??userId,updated_at:new Date().toISOString()};const result=selected?await supabase.from("work_orders").update(payload).eq("id",selected.id).select("id").single():await supabase.from("work_orders").insert(payload).select("id").single();if(result.error){setMessage(result.error.message);setBusy(false);return}const id=result.data.id;if(selected)await supabase.from("work_order_products").delete().eq("work_order_id",id);if(draft.products.length){const productResult=await supabase.from("work_order_products").insert(draft.products.filter(p=>p.product_name.trim()).map(p=>({group_id:groupId,work_order_id:id,input_id:p.input_id||null,product_name:p.product_name.trim(),dose:p.dose?Number(p.dose):null,dose_unit:p.dose_unit||null,planned_quantity:p.planned_quantity?Number(p.planned_quantity):null})));if(productResult.error){setMessage(productResult.error.message);setBusy(false);return}}setBusy(false);setMode(null);setSelected(null);onSaved()}
 ...
 const typeLabel=(v:string)=>({application:"Aplicación",sowing:"Siembra",harvest:"Cosecha",activity:"Actividad",monitoring:"Monitoreo",other:"Otra"} as Record<string,string>)[v]??v;
 ...
 return <div className="page-content work-orders-page"><PageHead title="Órdenes de trabajo" text="Planificá, asigná y convertí la ejecución en un registro real."/>
  ...
  <section className="content-card work-order-directory"><header><div className="work-order-search"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar por orden, campo o lote"/></div><select value={status} onChange={e=>setStatus(e.target.value)}><option value="active">Activas</option><option value="all">Todas</option><option value="pending">Pendientes</option><option value="in_progress">En curso</option><option value="completed">Completadas</option><option value="cancelled">Canceladas</option></select>{canCreate&&<button className="primary-action" onClick={openCreate}><Plus/>Nueva orden</button>}</header><div className="work-order-list">{filtered.map(item=><button key={item.id} onClick={()=>{setSelected(item);setMode("view");setMessage("")}}><i className={`priority-${item.priority}`}/><div><span>{typeLabel(item.order_type)} · {formatDate(item.scheduled_date)}</span><strong>{item.title}</strong><small>{item.fields?.name??"Sin campo"}{item.plots?.name?` · ${item.plots.name}`:""}{item.planned_area?` · ${number(item.planned_area).toLocaleString("es-AR")} ha`:""}</small></div><em className={`status-${item.status}`}>{statusLabel(item.status)}</em><ChevronRight/></button>)}{!filtered.length&&<EmptyLine text="No hay órdenes para este filtro."/>}</div></section>
  {mode&&<div className="record-detail-backdrop"><article className="record-detail-sheet work-order-sheet"><header><div><span className="eyebrow">ORDEN DE TRABAJO</span><h2>{mode==="form"?(selected?"Editar orden":"Nueva orden"):selected?.title}</h2></div><button className="icon-button" onClick={()=>{setMode(null);setSelected(null)}}><X/></button></header>
   {mode==="form"?<form className="work-order-form" onSubmit={save}><div className="form-grid"><label className="wide">Título<input required value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})} placeholder="Ej. Aplicar fungicida en Lote Norte"/></label><label>Tipo<select value={draft.order_type} onChange={e=>setDraft({...draft,order_type:e.target.value})}><option value="application">Aplicación</option><option value="sowing">Siembra</option><option value="harvest">Cosecha</option><option value="activity">Actividad</option><option value="monitoring">Monitoreo</option><option value="other">Otra</option></select></label><label>Prioridad<select value={draft.priority} onChange={e=>setDraft({...draft,priority:e.target.value})}><option value="low">Baja</option><option value="normal">Normal</option><option value="high">Alta</option><option value="urgent">Urgente</option></select></label><label>Fecha prevista<input required type="date" value={draft.scheduled_date} onChange={e=>setDraft({...draft,scheduled_date:e.target.value})}/></label><label>Hasta<input type="date" min={draft.scheduled_date} value={draft.scheduled_end_date} onChange={e=>setDraft({...draft,scheduled_end_date:e.target.value})}/></label><label>Campaña<select required value={draft.campaign_id} onChange={e=>setDraft({...draft,campaign_id:e.target.value})}><option value="">Seleccionar</option>{campaigns.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Campo<select value={draft.field_id} onChange={e=>setDraft({...draft,field_id:e.target.value,plot_id:""})}><option value="">Sin definir</option>{fields.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select></label><label>Lote<select value={draft.plot_id} onChange={e=>setDraft({...draft,plot_id:e.target.value})}><option value="">Sin definir</option>{plots.filter(p=>!draft.field_id||p.field_id===draft.field_id).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>Superficie planificada (ha)<input type="number" min="0" step="0.01" value={draft.planned_area} onChange={e=>setDraft({...draft,planned_area:e.target.value})}/></label><label>Responsable<select value={draft.assigned_to} onChange={e=>setDraft({...draft,assigned_to:e.target.value})}><option value="">Sin asignar</option>{members.map(m=><option key={m.user_id} value={m.user_id}>{[m.profiles?.first_name,m.profiles?.last_name].filter(Boolean).join(" ")||m.profiles?.username}</option>)}</select></label><label>Contratista<select value={draft.contractor_id} onChange={e=>setDraft({...draft,contractor_id:e.target.value})}><option value="">Sin contratista</option>{contractors.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label className="wide">Instrucciones<textarea value={draft.instructions} onChange={e=>setDraft({...draft,instructions:e.target.value})} placeholder="Qué debe hacerse y cómo"/></label></div><section className="work-order-products"><header><div><strong>Insumos planificados</strong><small>Dosis y cantidad quedan vinculadas a la orden.</small></div><button type="button" onClick={()=>setDraft({...draft,products:[...draft.products,{input_id:"",product_name:"",dose:"",dose_unit:"",planned_quantity:""}]})}><Plus/>Agregar</button></header>{draft.products.map((p,index)=><div key={index}><select value={p.input_id} onChange={e=>{const supply=supplies.find(s=>s.id===e.target.value);const next=[...draft.products];next[index]={...p,input_id:e.target.value,product_name:supply?.name??p.product_name,dose_unit:supply?.unit??p.dose_unit};setDraft({...draft,products:next})}}><option value="">Producto manual</option>{supplies.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select><input placeholder="Nombre" value={p.product_name} onChange={e=>{const next=[...draft.products];next[index]={...p,product_name:e.target.value};setDraft({...draft,products:next})}}/><input type="number" step="0.01" placeholder="Dosis" value={p.dose} onChange={e=>{const next=[...draft.products];next[index]={...p,dose:e.target.value};setDraft({...draft,products:next})}}/><input placeholder="Unidad" value={p.dose_unit} onChange={e=>{const next=[...draft.products];next[index]={...p,dose_unit:e.target.value};setDraft({...draft,products:next})}}/><button type="button" onClick={()=>setDraft({...draft,products:draft.products.filter((_,i)=>i!==index)})}><X/></button></div>)}</section>{message&&<p className="form-error">{message}</p>}<footer><button type="button" className="secondary-action" onClick={()=>setMode(null)}>Cancelar</button><button className="primary-action" disabled={busy}>{busy?<LoaderCircle className="spin"/>:<Save/>}Guardar orden</button></footer></form>
   :mode==="complete"?<form className="work-order-complete" onSubmit={complete}><div className="planned-actual"><article><small>PLANIFICADO</small><strong>{formatDate(selected!.scheduled_date)}</strong><span>{selected!.planned_area?`${number(selected!.planned_area).toLocaleString("es-AR")} ha`:"Sin superficie"}</span></article><ArrowRight/><article><small>REAL</small><label>Fecha<input required type="date" value={actualDate} onChange={e=>setActualDate(e.target.value)}/></label><label>Superficie (ha)<input type="number" min="0" step="0.01" value={actualArea} onChange={e=>setActualArea(e.target.value)}/></label></article></div>{plan!=="free"&&<div className="comparison-note"><TrendingUp/><span><strong>Comparación planificado vs. real</strong><small>{plan==="business"?"El plan Business conserva análisis avanzado y desvíos por recurso.":"El plan Pro conserva la comparación básica de fecha y superficie."}</small></span></div>}{message&&<p className="form-error">{message}</p>}<footer><button type="button" className="secondary-action" onClick={()=>setMode("view")}>Volver</button><button className="primary-action" disabled={busy}>{busy?<LoaderCircle className="spin"/>:<Check/>}Confirmar ejecución</button></footer></form>
   :selected&&<div className="work-order-detail"><div className="work-order-hero"><div><span className={`status-${selected.status}`}>{statusLabel(selected.status)}</span><h3>{typeLabel(selected.order_type)}</h3><p>{selected.instructions||"Sin instrucciones adicionales."}</p></div><span className={`priority priority-${selected.priority}`}>{selected.priority==="urgent"?"Urgente":selected.priority==="high"?"Alta":selected.priority==="low"?"Baja":"Normal"}</span></div><div className="work-order-facts"><article><CalendarDays/><small>Fecha prevista</small><strong>{formatDate(selected.scheduled_date)}</strong></article><article><Sprout/><small>Ubicación</small><strong>{selected.fields?.name??"Sin campo"}{selected.plots?.name?` · ${selected.plots.name}`:""}</strong></article><article><Users/><small>Responsable</small><strong>{[selected.profiles?.first_name,selected.profiles?.last_name].filter(Boolean).join(" ")||"Sin asignar"}</strong></article><article><Tractor/><small>Contratista</small><strong>{selected.contractors?.name??"Sin contratista"}</strong></article></div>{Boolean(selected.work_order_products?.length)&&<section><h3>Insumos planificados</h3><div className="work-order-product-list">{selected.work_order_products!.map(p=><div key={p.id}><strong>{p.product_name}</strong><span>{p.dose?`${number(p.dose).toLocaleString("es-AR")} ${p.dose_unit??""}/ha`:"Sin dosis"}</span><small>{p.planned_quantity?`${number(p.planned_quantity).toLocaleString("es-AR")} total`:""}</small></div>)}</div></section>}<footer className="work-order-actions"><button onClick={()=>window.print()}><Download/>PDF</button><button onClick={()=>void share(selected)}><Link2/>Compartir</button>{mayEdit(selected)&&selected.status!=="completed"&&selected.status!=="cancelled"&&<button onClick={()=>openEdit(selected)}><Settings2/>Editar</button>}{mayEdit(selected)&&selected.status==="pending"&&<button onClick={()=>void changeStatus(selected,"in_progress")}><Activity/>Iniciar</button>}{canCreate&&selected.status!=="completed"&&selected.status!=="cancelled"&&<button className="primary-action" onClick={()=>{setActualDate(new Date().toISOString().slice(0,10));setActualArea(selected.planned_area?String(selected.planned_area):"");setMode("complete")}}><Check/>Completar</button>}{mayEdit(selected)&&!["completed","cancelled"].includes(selected.status)&&<button className="danger-text" onClick={()=>void changeStatus(selected,"cancelled")}>Cancelar orden</button>}</footer>{message&&<p className="form-error">{message}</p>}</div>}
  </article></div>}
 </div>
}
...
function PlansView({plans,subscription,plots,campaigns,assignments,members,groupName}:{plans:SubscriptionPlan[];subscription:GroupSubscription|null;plots:Plot[];campaigns:Campaign[];assignments:PlotCampaign[];members:Member[];groupName:string}){
  ...
  const hectares=sum(plots.filter(plot=>activePlotIds.has(plot.id)).map(plot=>number(plot.arable_area||plot.total_area)));
  const metrics=[
