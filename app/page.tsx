...
function wrapPdfLine(value:string,max=88){const text=String(value??"").replace(/\s+/g," ").trim();if(!text)return[""];const words=text.split(" ");const lines:string[]=[];let line="";for(const word of words){const next=line?`${line} ${word}`:word;if(next.length>max&&line){lines.push(line);line=word}else line=next}if(line)lines.push(line);return lines}
function buildSimplePdf(title:string,rawLines:string[]){
  const lines=rawLines.flatMap(line=>wrapPdfLine(line));
  const chunks:string[][]=[];for(let i=0;i<lines.length;i+=48)chunks.push(lines.slice(i,i+48));if(!chunks.length)chunks.push([]);
  const objects:string[]=[];const add=(value:string)=>{objects.push(value);return objects.length};
  const catalogId=add("");const pagesId=add("");const fontId=add("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>");
  const pageIds:number[]=[];
  chunks.forEach((chunk,pageIndex)=>{const body=["BT","/F1 16 Tf","50 800 Td",`(${pdfEscape(pageIndex===0?title:`${title} · ${pageIndex+1}`)}) Tj`,"0 -26 Td","/F1 9.5 Tf"].concat(chunk.flatMap(line=>[`(${pdfEscape(line)}) Tj`,`0 -14 Td`])).concat(["ET"]).join("\n");const contentId=add(`<< /Length ${pdfLatin1(body).length} >>\nstream\n${body}\nendstream`);const pageId=add(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${fontId} 0 R >> >> /Contents ${contentId} 0 R >>`);pageIds.push(pageId)});
...
function recordPdfLines(row:RecordRow){const details=recordData(row);const lines=[`Tipo: ${recordType(effectiveRecordType(row))}`,`Fecha: ${row.record_date?formatDate(row.record_date):"—"}`,`Campo: ${relation(row.fields)?.name??"—"}`,`Lote: ${relation(row.plots)?.name??"—"}`,`Campaña: ${relation(row.campaigns)?.name??"—"}`,`Cultivo: ${recordCrop(row)||"—"}`,`Superficie trabajada: ${row.worked_area!=null?`${number(row.worked_area).toLocaleString("es-AR",{maximumFractionDigits:2})} ha`:"—"}`];if(row.contractor)lines.push(`Contratista: ${row.contractor}`);if(row.machinery_text)lines.push(`Maquinaria: ${row.machinery_text}`);lines.push("","DATOS DEL REGISTRO");Object.entries(details).filter(([,value])=>value!==null&&value!==undefined&&String(value).trim()!=="").forEach(([key,value])=>{let rendered:string;if(typeof value==="boolean")rendered=value?"Sí":"No";else if(value&&typeof value==="object")rendered=JSON.stringify(value);else rendered=translatedDetailText(value);lines.push(`${detailLabel(key)}: ${rendered}`)});if(row.observations)lines.push("","OBSERVACIONES",row.observations);lines.push("","Documento generado con Growr360 · growr360.com");return lines}
